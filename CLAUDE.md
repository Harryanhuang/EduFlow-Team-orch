...
This is the `rebuild/minimal` branch — a clean-slate EduFlow rewrite.
...
Business logic lives in `src/eduflow/` only.  There is **no** parallel
`scripts/` shim layer (that was the old branch's biggest source of
double-residency).  Every command is a Python module under
`src/eduflow/commands/` registered in `src/eduflow/cli.py`.

```
src/eduflow/cli.py              ← top-level dispatch + COMMANDS registry
src/eduflow/commands/X.py       ← one module per subcommand, ~30 LOC each
src/eduflow/store/              ← file-backed local state (no DB)
...
- **Env-driven state**: `runtime/paths.state_dir()` re-reads
  `$EDUFLOW_STATE_DIR` on every call.  Never cache it at module load.
...
- **Pure functions where possible**: `feishu/router.classify_event`,
  ...
  given their inputs.
- **One file per `eduflow` subcommand**: don't grow a 900-line
...
- Don't put business logic under `scripts/` at the repo root.
  ...
  `eduflow = "eduflow.cli:main"`. The only thing allowed under
  ...
  their own `package.json` / runtime and never import eduflow.
...
8. (done) `eduflow health`, `up`, `down`, agent heartbeats
9. (done) Slash command hooks (`eduflow install-hooks` → .claude/commands/)
10. (done) `eduflow usage` — ccusage wrapper for claude-code agents
11. (done) Rate-limit detection (adapter `rate_limit_markers`, deliver skips)
12. (done) `eduflow reset` + 15-helper `util.py` shared stdlib
...
19. (done) Multi-team isolation UX (`eduflow switch <team-dir>` emits shell exports)
...
24. (done) Memory CRUD CLI: `eduflow remember` / `recall` / `forget` — R172.b retired the matching `/recall` and `/forget` slash dispatch (boss-flagged not-in-main); CLI form stays for agent-pane use
...
28. (done) `eduflow reidentify --all` for batch re-injection across the team
29. (done) Watchdog cooldown alert promoted to red Feishu card with recovery checklist (was plain text)
30. (done) `eduflow say <agent> <msg> --card` for card-formatted chat replies; manager → blue / worker_* → green template by convention
