...
1. IGCSE course task 现在会强制挂载 `workflow_id=igcse-subject-launch`；CLI 兼容 `--stage course`，内部仍 canonical 到 `curriculum`。
2. `task get` 和 `task workflow-status <id>` 都能显示当前 workflow gate、gate status、default reviewer、next action。
3. `task submit-review` 会对 `igcse-subject-launch` 自动补默认 reviewer=`review_course`。
4. `task review` 已把 scope/evidence/verdict truth 写入结构化字段。
5. `task batch-closeout <id> --actor manager` 已新增，且与 `task manager-closeout` 严格分开。
6. `task manager-closeout` 继续只允许 subject closeout，batch/package 会被明确拒绝。
7. `task scan-anomalies` 已覆盖：
   - `workflow_mentioned_but_not_mounted`
   - `submitted_for_review_without_reviewer`
   - `review_pass_log_but_task_pending`
   - `package_pass_promoted_to_subject_closeout`
...
- 没有在这个包里实现“从可见 PASS 日志自动反写 task truth”；当前先做到结构化识别 anomaly，而不是自动修正。
- 没有扩展 runtime/router/QBank 相关修复，按包边界保持不动。

当前风险：

- 现有仓库里若还有历史脚本只写 `curriculum` 口径，行为仍兼容，但对外文档已改成 `IGCSE course`，后续包最好逐步统一 operator 口径。
- `package` 与 `subject` 的区分目前主要靠 title/scope/verdict_target 的结构化信号；如果历史任务命名很松散，仍可能需要人工补 scope。
...
- 已完成：`read --ack completed` / `read --ack reconciled` 已落地，help 与实现一致。
- 已完成：高优消息状态语义已补齐到 `pending / agent_acknowledged / action_started / completed / reconciled`。
- 已完成：`--no-inject` 高优消息会记录 `delivery_state=requires_polling`，`inbox` 可见，supervisor 会单独归类。
- 已完成：supervisor 对“消息仍 unread，但后续日志/状态已证明执行”的情况降级为 `read_state_desync`，不再继续报 live blocker。
- 已完成：`read=true but ack missing` 仍单独保留为 `high_priority_inbox_read_without_ack` / `ack_semantics_gap`。
- 已完成：`send` 的成功证据仍以 `local_id` 为主，注入只作为 `delivery_state` 观测，不再等同于消费成功。
- 已完成：`say --to ... --body ...` 现有解析与测试已覆盖，不再出现 `agent=\"--body\"`。

剩余风险：

- 目前只实现了最小 reconcile 路径：`read <msg_id> --ack reconciled`；尚未补独立 `inbox reconcile <msg_id> --evidence-log <log_id>` 子命令。
- `delivery_state` 目前是轻量观测字段，尚未和更细的 runtime/router 证据自动联动；这部分应留给包 3/4 继续收口。
