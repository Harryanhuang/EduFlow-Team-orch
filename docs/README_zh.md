...
<p align="center">
  <img src="media/hero.png" alt="EduFlow — 动态 AI 团队编排" width="880" />
</p>
...
>
> ```
> 克隆 https://github.com/zylMozart/EduFlow.git，读 docs/DEPLOYMENT.md，
> 然后从头到尾带我跑一次 EduFlow (如果还没飞书 App 也帮我建一个)。
> ```
...
<p align="center">
  <img src="media/architecture.png" alt="EduFlow 架构图：飞书 Router 把消息派给 Manager Agent；Manager 用 /hire /fire 动态控制 Worker 池（Claude Code / Codex / Kimi / Gemini / Qwen 各跑独立 tmux pane，每个有自己的 identity / memory / workspace / 任务队列）；Watchdog 自动重启失败进程；Shared State 在飞书 Bitable + tasks + status board + memory。" width="880" />
</p>
...
- 🐛 **GitHub issue** — [在这里开](https://github.com/zylMozart/EduFlow/issues/new/choose)，请附上 OS、部署模式 (host / Docker)、失败命令的输出。
- 💬 **微信交流群** — 扫下面二维码（每周刷新）。
...
- **单一配置文件** — `eduflow.toml`（Cargo 风格，可写注释），chat_id / agents / 模型 / 卡片色 / publish 过滤都在一起。
...
# 配置
eduflow init                  # 写 eduflow.toml
$EDITOR eduflow.toml          # 填 chat_id 和 agents
eduflow install-hooks         # 装 claude-code 斜杠命令钩子

# 启动
eduflow up                    # tmux + agents + router + watchdog
eduflow health                # 查 green/yellow/red
```
...
EduFlow 需要一个飞书自建应用 + 一组权限 + 事件订阅 + 卡片回调。两条路：
...
node create_feishu_bot.js drive my-bot "我的 EduFlow 机器人" \
  > /tmp/drive.log 2>&1 &
...
```bash
node create_feishu_bot.js create my-bot "我的 EduFlow 机器人"
node create_feishu_bot.js batch bots.json     # [{name, description}, ...]
```

完工后把 `App ID` + `App Secret` 填进 `.env` (Docker) 或 `eduflow.toml`，加上群 `chat_id`。
...
**Q：能跑非 Anthropic 模型吗？**
A：能。多 CLI 适配表如上。每个员工在 `eduflow.toml` 里挑自己的 `cli`。

**Q：能用 Slack / Discord 替飞书吗？**
A：开箱不行。Chat 层是飞书绑定的 (`src/eduflow/feishu/`)。

**Q：能跑多少个 agent？**
A：测试到 5 个。每个 Claude Code pane ~200–400 MB；8 GB 物理内存跑 5 个轻松。

**Q：员工挂了上下文会丢吗？**
A：不会。inbox + status + logs + durable memory 都在本地磁盘。看门狗自动重生 daemon；`eduflow reidentify <agent>` 重灌身份 prompt 时自动加载历史 memory。

**Q：要花多少钱？**
A：EduFlow 本身 MIT 协议免费。开销来自 CLI 后端的 API 调用费。飞书 + `lark-cli` 都免费。
