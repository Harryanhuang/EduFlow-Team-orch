...
End-to-end setup for EduFlow. Covers host-native, Docker, config,
...
2. **Get the bot into a Feishu group**. Self-built apps cannot be
   ...
   **Path B — manual add to an existing group** (if you want to drop
   ...
   `eduflow.toml` in step 4 below. Skip this step entirely and every
   `eduflow say` will fail with `code=230001 invalid receive_id`.
...
4. **Config** — write the credentials into `.env` (Docker) and the
   `chat_id` + agents into `eduflow.toml`. `eduflow init`
   generates a starter `eduflow.toml` with three default agents
...
5. **Launch + verify** — `eduflow up` then `eduflow health`
...
cd /path/to/EduFlow
export EDUFLOW_STATE_DIR="$PWD/state"   # else state goes to ~/.eduflow
...
eduflow init
$EDITOR eduflow.toml                    # set chat_id + add agents

# 4. install slash hooks BEFORE up (claude-code caches them at pane spawn)
eduflow install-hooks                   # writes .claude/commands/<name>.md

# 5. bring up the team
eduflow up                              # tmux session + agents + router + watchdog
eduflow health                          # verify everything green/yellow
```

**Tear down:**

```bash
eduflow down       # stop panes + daemons, keep inbox/logs/cursor
eduflow reset      # nuclear option: also wipes state
```
...
You don't need Python or `eduflow` on the host — everything runs
...
docker compose exec eduflow eduflow install-hooks
docker compose exec eduflow eduflow up
docker compose exec eduflow eduflow health
docker compose exec eduflow tmux attach -t EduFlow   # see panes; Ctrl+B d to leave
```
...
> (`router.stale_event_threshold_s = N`) or env (`EDUFLOW_ROUTER_STALE_S=N`)
...
[team]
session = "EduFlow"                        # tmux session name
...
**Override precedence** (highest wins): `env` > `eduflow.toml` > code default.
See `src/eduflow/runtime/tunables.py` for the cascade.
...
| ------- | ------------ | ------------------------------- |
| `eduflow send <to> <from> <msg>` | Append a row to local `inbox.json` | **No** — only `eduflow inbox <to>` reads it |
| `eduflow say <agent> "<msg>" --to <role>` | Post into Feishu chat (subject to `[chat.publish]`) | Only if router relays it back |
...
```bash
# team A
export EDUFLOW_STATE_DIR=/path/to/team-a/state
cd /path/to/team-a
eduflow up   # session "TeamA"

# team B (different shell)
export EDUFLOW_STATE_DIR=/path/to/team-b/state
cd /path/to/team-b
eduflow up   # session "TeamB"
```
...
routing conflicts. `eduflow switch <team-dir>` emits the env
...
After `eduflow install-hooks`, the manager pane recognises these:
...
| `/stop <agent>`       | Kill pane (re-spawn with `eduflow hire <agent>`) |
...
After `eduflow up` returns green:
...
### Manager loops on the same anchored message after `eduflow up`
...
### `eduflow say` from a pane fails HTTP 400 "Bot/User can NOT be out of the chat"

Symptom: `eduflow say` from your launching shell **works**, but the
...
**server** started by an earlier `eduflow up` (different checkout,
...
`LARKSUITE_CLI_APP_*` + `EDUFLOW_STATE_DIR` directly into each
spawn-cmd, so this should no longer trigger from a clean state. If you
still see it, the orphan-tmux trap is the cause:

```bash
# 1. surface stale tmux servers + orphan EduFlow daemons
tmux ls 2>/dev/null
ps -ef | grep -E "eduflow (router|watchdog)|lark-cli.*subscribe" | grep -v grep

# 2. clean up
eduflow down                           # graceful local stop
...
tmux kill-session -t EduFlow

# 3. relaunch from a shell that has the right env exported
eduflow up
tmux show-environment -g | grep -E "FEISHU_APP_ID|EDUFLOW_STATE_DIR"  # verify
```
...
```bash
tmux send-keys -t EduFlow:worker_codex 3 Enter   # picks "Skip until next version"
eduflow reidentify worker_codex
```
...
| Command | Purpose |
| ------- | ------- |
| `eduflow up` / `down` | Bring team up / take it down |
| `eduflow health` | One-shot status (binaries, env, tmux, daemons, cursor, memory) |
| `eduflow team` | Each agent's state + ♥ heartbeat |
| `eduflow peek <agent> [N]` | Pane snapshot for the 5-min check-in cadence |
| `eduflow reidentify [<agent> \| --all]` | Re-inject identity.md (after prompt change) |
| `eduflow usage [--days N]` | ccusage wrapper for claude-code agents |
| `eduflow say <agent> "<msg>" --to <role>` | Post as agent into the chat |
| `eduflow remember <agent> <kind> "<note>"` | Write durable memory (auto-injected on next pane wake) |
| `eduflow switch <team-dir>` | Print env exports for multi-team UX |

`eduflow --help` lists everything grouped by section.
