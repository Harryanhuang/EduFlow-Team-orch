...
def _config(path, errors=None, subject="config"):
    ...
    if raw is None:
        if errors is not None:
            _err(errors, "config_unreadable", subject, "actual config cannot be read")
        return {
            "path": str(path),
            "sha256": None,
            "generation": None,
    ...
    try:
        ...
        data = {}
        if errors is not None:
            _err(errors, "config_corrupt", subject, "TOML cannot be parsed")
    team, registry, invalid = (
        data.get("team", {}),
        data.get("runtime_registry", {}),
        False,
    )
    ...
    if not text(profile) or not text(session):
        invalid = True
    if any(
        not text(name) or not isinstance(value, dict) or not text(value.get("cli"))
        for name, value in registry.items()
    ):
        invalid = True
    if any(
        not text(name) or not isinstance(value, dict) for name, value in agents.items()
    ):
        invalid = True
    agent_clis = {}
    for name, spec in agents.items():
        if not isinstance(spec, dict) or "runtime" not in spec:
            continue
        runtime = spec.get("runtime")
        if not text(runtime) or runtime not in registry:
            invalid = True
            continue
        node = registry[runtime]
        if not isinstance(node, dict) or not text(node.get("cli")):
            invalid = True
            continue
        agent_clis[name] = node["cli"]
    if invalid and errors is not None:
        _err(
            errors,
            "config_schema_invalid",
            subject,
            "config properties have invalid types",
        )
    return {
        "path": str(path),
        "sha256": digest,
...
def _git(deps, cwd, errors, subject):
    ...
    if not checkout:
        _err(errors, "checkout_unknown", subject, "git checkout unavailable")
    if not revision:
        _err(errors, "revision_unknown", subject, "git revision unavailable")
    if not checkout and not revision:
        _err(errors, "git_unavailable", subject, "git facts unavailable")
...
def _entry(command, executable):
    ...
    index = next(
        (
            ...
            if token == executable or Path(token).name == name
        ),
        None,
    )
    if index is None:
        return None, None, None
    if "python" in name.lower() and tokens[index + 1 : index + 3] == [
        "-m",
        "eduflow.cli",
    ]:
    ...
    if (
        ...
        and Path(tokens[index + 1]).name == "hermes"
        and tokens[index + 2] == "chat"
    ):
...
def _setting(command, names):
    for env, option in names:
        match = re.search(
            rf"(?:^|\s){re.escape(env)}=(.*?)(?=\s+[A-Z][A-Z0-9_]*=|"
            rf"\s+(?:python\d*(?:\.\d+)?|claude|codex|qoderclicn)\s|$)",
            command,
        )
        if match:
            return match.group(1).strip(" '\"")
        match = re.search(
            rf"(?:^|\s){re.escape(option)}(?:=|\s+)(.*?)(?=\s+--[\w-]+|$)", command
        )
        if match:
            return match.group(1).strip(" '\"")
        tokens = _tokens(command)
...
def _hermes_distribution(deps, python, script):
    ...
    if not isinstance(proof, dict):
        return None, False
    scripts, version, entrypoint = (
        proof.get(key) for key in ("scripts", "version", "entrypoint")
    )
    valid = all(
        isinstance(value, str) and bool(value.strip())
        for value in (scripts, version, entrypoint)
    )
    valid = (
        valid and len(version) <= 128 and "\n" not in version and "\r" not in version
    )
    if not valid:
        return None, False
...
def _provenance(deps, process, processes, target, errors, subject, profile):
    ...
    combined = (
        (env_result.stdout or "") + " " + " ".join(item["command"] for item in chain)
    )
    cwd = _cwd(deps, pid)
    ...
    if not config_path:
        _err(
            errors,
            "process_config_unknown",
            subject,
            "actual process config is not proven",
        )
    if not state_dir:
        _err(
            errors,
            ...
            subject,
            "actual process state dir is not proven",
        )
    actual = (
        _config(config_path, errors, subject)
        if config_path
        else _config(target["path"])
    )
    if checkout != target["checkout"]:
        _err(
            errors,
            "process_checkout_mismatch",
            subject,
            "actual checkout differs from target",
        )
    if actual["path"] != target["path"]:
        _err(
            errors,
            "process_config_mismatch",
            subject,
            "actual config differs from target",
        )
    if state_dir and str(Path(state_dir).resolve()) != target["state_dir"]:
        _err(
            ...
            subject,
            "actual state differs from target",
        )
    if (
        root
        and config_path
        and not Path(config_path).resolve().is_relative_to(Path(root).resolve())
    ):
        _err(
            errors,
            "process_root_config_conflict",
            subject,
            "actual config is outside EDUFLOW_ROOT",
        )
    if (
        root
        and state_dir
        and not Path(state_dir).resolve().is_relative_to(Path(root).resolve())
    ):
        _err(
            errors,
            "process_root_state_conflict",
            subject,
            "actual state is outside EDUFLOW_ROOT",
        )
    if actual["sha256"] != target["sha256"]:
        _err(
            errors,
            "process_config_generation_mismatch",
            subject,
            "actual config hash differs",
        )
    runtime = _version(deps, executable)
    hermes = cli_executable and Path(cli_executable).name == "hermes"
    cli_version, binding_mismatch = (
        _hermes_distribution(deps, executable, cli_executable)
        if hermes
        else (runtime, False)
    )
    if binding_mismatch:
        _err(
            errors,
            "hermes_distribution_mismatch",
            subject,
            "Hermes script is outside Python distribution scripts",
        )
    if not entry:
        _err(errors, "process_entry_unknown", subject, "strict EduFlow entry not found")
    if not runtime or not cli_version:
        _err(errors, "cli_runtime_unknown", subject, "executable version unavailable")
    return {
        "pid": pid,
        ...
        if executable and "python" in executable.casefold()
        else "not-applicable",
        "cli_runtime": f"{cli_executable} {cli_version}"
        if cli_executable and cli_version
        else None,
        ...
        "lark_profile": actual["lark_profile"],
        "tmux_session": actual["tmux_session"],
...
def _tmux(deps, session, errors):
    ...
    for line in result.stdout.splitlines():
        ...
        if len(parts) != 7:
            ...
            continue
        row = dict(
            zip(("session", "window", "index", "pid", "runtime", "cwd", "start"), parts)
        )
        try:
            row["pid"] = int(row["pid"])
        except ValueError:
...
def audit(deps):
    ...
    verified_checkout, verified_revision = _git(
        deps, requested_checkout, errors, "target"
    )
    if verified_checkout != requested_checkout:
        _err(
            ...
            "target",
            "requested checkout differs from git",
        )
    target = {
        **target_cfg,
    ...
    if not target["lark_profile"]:
        _err(errors, "lark_profile_missing", "config", "Lark profile is required")
    if not target["tmux_session"]:
        _err(errors, "tmux_session_missing", "config", "tmux session is required")
    ...
    for name in DAEMONS:
        ...
        try:
            ...
            _err(
                errors,
                "daemon_pid_corrupt" if raw else "daemon_pid_missing",
                name,
                "invalid PID file",
            )
        process = processes.get(pid) if pid else None
        if pid and not process:
            _err(errors, "daemon_pid_not_live", name, "PID absent from ps")
        if process and _entry(process["command"], _executable(deps, pid))[1] != name:
            _err(errors, "daemon_entry_drift", name, "PID has a different strict entry")
        if pid:
            expected.add(pid)
        ...
        if not process:
            proof.update(pid=pid, daemon_profile=profile)
    ...
    for pane in pane_rows:
        ...
        if not process:
            _err(
                errors, "agent_process_missing", subject, "no strict agent child found"
            )
        expected_cli = target["agent_clis"].get(pane["window"])
        actual_cli = (
            Path(_entry(process["command"], _executable(deps, process["pid"]))[2]).name
            if process
            else None
        )
        if not expected_cli:
            _err(
                errors,
                "agent_runtime_mapping_missing",
                subject,
                "configured CLI is unknown",
            )
        elif _cli_name(expected_cli) != _cli_name(actual_cli):
            _err(
                errors,
                "agent_entry_drift",
                subject,
                "actual CLI differs from configured runtime",
            )
        proof = _provenance(
            deps, process, processes, target, errors, subject, "tmux-agent"
        )
        if proof["checkout_source"] != "EDUFLOW_ROOT" and (
            not Path(pane["cwd"]).is_absolute()
            ...
            _err(
                ...
                "pane_cwd_drift",
                subject,
                "pane cwd differs without proven EDUFLOW_ROOT",
            )
        if process and proof["checkout"] != target["checkout"]:
            suspects.append(
                {
    ...
    for process in processes.values():
        ...
        if action in DAEMONS and process["pid"] not in expected:
            suspects.append(
                {"kind": "duplicate_daemon", "pid": process["pid"], "entry": entry}
            )
            _err(
                errors,
                "duplicate_daemon",
                str(process["pid"]),
                "unexpected daemon process",
            )
            ...
            context = (
                (env.stdout or "") + " " + " ".join(row["command"] for row in chain)
            )
            actual_config = _setting(context, (("EDUFLOW_CONFIG_FILE", "--config"),))
            actual_state = _setting(context, (("EDUFLOW_STATE_DIR", "--state-dir"),))
            scoped = (
                "eduflow.cli agent" in (entry or "")
                or _cwd(deps, process["pid"]) == target["checkout"]
                or bool(pane_roots & {row["pid"] for row in chain})
                or (
                    actual_config
                    and str(Path(actual_config).resolve()) == target["path"]
                )
                or (
                    actual_state
                    and str(Path(actual_state).resolve()) == target["state_dir"]
                )
            )
            if scoped:
                suspects.append(
                    {"kind": "orphan_agent", "pid": process["pid"], "entry": entry}
                )
                _err(
                    errors,
                    "orphan_agent",
                    str(process["pid"]),
                    "agent is not linked to configured pane",
                )
        elif (
            Path(_tokens(process["command"])[0]).name != "tmux"
            and re.search(
                r"(?:eduflow\.cli|(?:^|[/\s])eduflow(?:team)?\s)",
                process["command"].lower(),
            )
            and not action
        ):
            suspects.append(
                {"kind": "legacy_entry", "pid": process["pid"], "entry": "unrecognized"}
            )
            _err(
                errors,
                "legacy_entry",
                str(process["pid"]),
                "unrecognized EduFlow command",
            )
    raw_commands = (
        " ".join(p["command"] for p in processes.values())
        + " "
        + " ".join(p["start"] for p in pane_rows)
    )
...
def _default_dependencies(args):
    ...
    config = (
        args.config
        or os.environ.get("EDUFLOW_CONFIG_FILE")
        or checkout / "eduflow.toml"
    )
    state = (
        args.state_dir
        or os.environ.get("EDUFLOW_STATE_DIR")
        or Path.home() / ".eduflow"
    )
    return AuditDependencies(
        checkout,
        Path(config).expanduser().resolve(),
        Path(state).expanduser().resolve(),
        _default_run,
...
def main(argv=None, *, deps=None):
    if deps is None:
        ...
        parser.add_argument(
            "--checkout", default=str(Path(__file__).resolve().parents[1])
        )
        for flag in ("--config", "--state-dir", "--expected-config-sha256"):
