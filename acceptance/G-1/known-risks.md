...
| Medium | Mandatory Ruff, type-checker, secret-scanner, and `pip-audit` evidence is absent; Node audit fails because no approved lockfile exists. | security owner | approved tooling and Node policy checkpoint before Gate PASS | AC-GLOBAL-04 cannot pass | approve exact tools, versions, scopes, Node package-manager/lockfile policy, and audit threshold; then execute without inventing results |
