...
- 部署上线（venv 激活、`eduflow up`、`eduflow health`）
...
```bash
eduflow up        # 起 tmux 会话 + router + watchdog
eduflow health    # 三个 agent ✅，router 与 watchdog 都活着
```
...
- "pane up but CLI not ready"——常见原因是 codex 弹更新提示。`tmux capture-pane -t EduFlow:worker_codex -p` 看一眼，按 `3 Enter` 选「Skip until next version」即可
...
- 群里 manager 没回——manager pane 卡住或没在工作。先 `tmux capture-pane -t EduFlow:manager -p | tail -30` 看 LLM 状态；再看 `state/router.log` 看路由是不是 ROUTE 到 manager
...
eduflow say worker_cc "$ANCHOR" --card
```
...
- 只看到 worker_cc 卡，半分钟后没 manager 卡——R174 的 worker→manager
  反向分支没生效，或 manager 卡住了。先 `eduflow inbox manager` 看消息
...
某些 worker 配 `"lazy": true` 时，`eduflow start` 不真起 CLI；首条
...
eduflow say worker_cc "test→user [$ANCHOR]" --to user
eduflow say worker_cc "test→manager [$ANCHOR]" --to manager
sleep 5
...
2. 把 `eduflow.toml` 的 `worker_to_manager = false`，重启 router：
   ```bash
   sed -i.bak 's/worker_to_manager = true/worker_to_manager = false/' eduflow.toml
   kill $(cat state/router.pid); eduflow up
   ANCHOR2="publish-silenced-$(date +%s)"
   eduflow say worker_cc "test→manager silenced [$ANCHOR2]" --to manager
   eduflow say worker_cc "test→user OK [$ANCHOR2]" --to user
   ```
3. 群里**只看到 →user 那张卡**，→manager 那条不进群（CLI 会打
   `📝 worker_cc → silenced by [chat.publish.worker_to_manager]=false; logged only`）
4. 还原：`mv eduflow.toml.bak eduflow.toml; kill $(cat state/router.pid); eduflow up`
...
LARK_CLI_NO_PROXY=1 lark-cli im +messages-send --chat-id "$CHAT" \
  --text "数 src/eduflow/feishu/ 下 .py 个数 [$ANCHOR3]" --as user
sleep 90
tmux capture-pane -t EduFlow:manager -p -S -50 | grep -A1 "eduflow say"
tmux capture-pane -t EduFlow:worker_cc -p -S -50 | grep -A1 "eduflow say"
```

**通过条件**：两个 pane 的 `eduflow say ...` 命令行尾都有 `--to user`
或 `--to manager`，不是裸 `eduflow say <agent> "..."`。如果发现裸 say，
identity prompt 没生效；rerun `eduflow reidentify --all` 后再试，
...
只停 pane 和守护进程，不会删收件箱、日志、游标。要彻底清空：`eduflow reset`（看 [_archive/team_down_and_reset.md](_archive/team_down_and_reset.md)）。
...
2. **重新部署后 worker pane 可能「Not logged in」**——claude 续期 token 时只更新 keychain，每个 agent home 下的 `state/agent-home/<agent>/.claude/.credentials.json` 是当时快照，过几天会过期。临时解：`eduflow down && eduflow up`，让 lifecycle 从 keychain 重新物化一遍
3. **codex 启动可能弹更新框**——挡住 ready marker 60 秒超时。手动 `tmux send-keys -t EduFlow:worker_codex 3 Enter` 选 Skip-until-next，再 `eduflow reidentify --all`
4. **第一次 user OAuth 之后，每个新 shell 仍要 `export` 那 4 个环境变量**——没持久化的话 `eduflow say` 偶尔会走 user 身份失败
...
- agent 之间互相发信（`eduflow send worker_a worker_b "..."`）：看 [local_message_cycle.md](local_message_cycle.md)
