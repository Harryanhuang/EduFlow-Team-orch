...
把 EduFlow 跑在容器里，state + 多种 OAuth credentials 通过 volume
...
docker compose up -d
docker compose exec eduflow eduflow install-hooks   # writes .claude/commands/*.md
docker compose exec eduflow eduflow up
docker compose exec eduflow eduflow health
docker compose exec eduflow tmux attach -t ContainerA  # eyeball panes
```
...
- ✅ 退出 `docker compose down` 后再 `up`，team-data 持久化、`eduflow health`
...
4. **CMD 是 sleep infinity，不是 `eduflow up`** — `up` 让 tmux 起
...
- **多容器编排**：每个 agent 各自一容器、router 单独一容器 etc. ——
  现在的 EduFlow runtime 把 router/watchdog/panes 共享一个 tmux
