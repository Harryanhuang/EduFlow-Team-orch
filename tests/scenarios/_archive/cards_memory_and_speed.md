...
4. **Memory CRUD slice trio** (R87/R92/R96/R107/R108/R111/R112), CLI
   ...
   - `eduflow remember <agent> <kind> "<content>" [--ref X]` — write
   - `eduflow recall <agent> [--limit N] [--kind K] [--json]` — read
   - `eduflow forget <agent> [--kind K] --yes` — drop
...
6. **`eduflow say <agent> <msg> --card`** (R99) for card-formatted
   chat replies; manager → blue, worker_* → green template by
   convention. `_color_for(agent)` is the shared helper.

7. **`eduflow peek <agent> [N]`** (R103) — branded fast path for
...
9. **Structured `--help`** (R93): `eduflow --help` renders commands
...
distinction. Manager 巡视 line now uses `eduflow peek <agent>`
...
- `eduflow up` started a team with at least 1 agent (manager + ≥0 workers).
...
- Header in **blue**, title `🆘 EduFlow 自定义斜杠命令`
...
`/health` → green card if `eduflow health` output has no ❌/⚠️ glyph,
...
```bash
# Make `eduflow router` fail at startup (corrupt runtime_config)
mv runtime_config.json runtime_config.json.bak
eduflow router  # exits non-zero immediately, no pid file
...
#   respawns. `eduflow health` for current state; check daemon log
#   for root cause.
```
...
eduflow remember manager decision "use bcrypt for password hashing" --ref om_xx

# Worker remembers a blocker
eduflow remember worker_cc blocker "missing GH PAT for push" --ref T-9

# After /clear or `eduflow reidentify <agent>`, the next init
...
# Or set EDUFLOW_LARK_CLI_BIN=/path/to/lark-cli explicitly.
```
...
| `/health` body | Code-fenced raw `eduflow health` output | Empty / unfenced — _shell stderr, check eduflow on PATH |
...
| memory.jsonl | One JSON record per `eduflow remember` | Missing — agent dir not created (ensure facts_dir() reachable) |
...
- Profile token expired → send returns `need_user_authorization` for `--as user`; for `--as bot`, may still work if bot perms are intact. Set `EDUFLOW_LARK_SEND_AS=bot` in env.
- Router daemon predates the rsync → it's still running OLD slash.py without dict-return support. Symptom: cards arrive as plain text. Fix: `eduflow down && up` to force daemon reload (Python doesn't hot-reload).
- `eduflow remember` writes to `~/.eduflow` instead of project state → `EDUFLOW_STATE_DIR` env not set in the calling shell. Identity init prompt's `cd` rule prevents this for spawned panes; risk is when an operator runs the command from a different cwd.

## Out of scope

- Single-entry pruning (operator dropping ONE specific entry by id from
  the middle): not exposed yet. `eduflow forget <agent> [--kind K]
