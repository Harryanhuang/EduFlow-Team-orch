...
- `eduflow up` 已把 team + router + watchdog 都拉起来
- `eduflow health` 显示 router/watchdog 都 alive
...
kill -9 $(cat $EDUFLOW_STATE_DIR/router.pid)
...
ps -eo pid,ppid,command | grep '@larksuite/cli.*+subscribe' | grep -v grep
# 期望: 只有一行，PPID 是新 router 的 pid（可在 $EDUFLOW_STATE_DIR/router.pid 读到）
```
...
| `eduflow health` | router alive，watchdog alive |
...
- 跨用户 ps（pid 属于别的 uid）→ `os.kill(pid, SIGTERM)` 抛 PermissionError，被吞掉，
  不会算作已 reap。这种情况只会在多用户 host 多账号跑 eduflow 时出现。

## Why this is here

round-65 之前老板在主机上手工 `kill -9 $(cat router.pid)` 测过守护重启 — 看上去 OK
（`eduflow health` 全绿），实际飞书事件已经 50/50 分裂。Bug 只在群里发足够多消息
