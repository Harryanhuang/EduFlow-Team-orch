# `eduflow usage`: token / credit snapshot

## 场景
老板想知道这一天 / 这一月 团队烧了多少 token、多少 credit。`eduflow usage` 用一条命令打完整摘要，按 CLI 维度分组：claude-code 走第三方 `ccusage` 工具（读 ~/.claude/projects 日志），其它 CLI 没上游工具就明说"去看你自己的 dashboard"。
...
# 月度
eduflow usage --view monthly

# 不同 view
eduflow usage --view session
eduflow usage --view blocks
```
...
## 反例
- `eduflow usage --bogus` → exit 1，stderr 含 `unexpected args`
- `eduflow usage --view foo` → exit 1，stderr 含 `unknown view`
