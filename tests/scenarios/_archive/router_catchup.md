...
- 凭证：飞书 bot 凭证就够，不需要 user OAuth。提交 `780fd08` 之后
  catchup 自动尊重 `EDUFLOW_LARK_SEND_AS=bot`，调用 `chat-messages-list`
...
- `EDUFLOW_LARK_SEND_AS=bot`（如果是 bot-only 部署）
- 旧的 `router.cursor`（如果存在）记录了某条早于待回放消息的 timestamp
- `eduflow start` 已经把 tmux 起好
...
# 在群里发"hello A"
sleep 5
cat $EDUFLOW_STATE_DIR/router.cursor    # 应记到 hello A 的 message_id
...
2. `eduflow inbox manager` 拿到这两条："missed B" 和 "@worker_codex check C"
   （**两条都到 manager**，R174 后 @worker_codex 不再分流）
3. `eduflow inbox worker_codex` 没有新行
