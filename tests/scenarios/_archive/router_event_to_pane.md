...
- `eduflow start` 已经把 tmux 会话和 manager pane 起好，CLI 横幅已出
- 飞书群里没有未读消息
...
2. 5 秒内（lark-cli 推送典型延迟），`eduflow inbox manager` 多出一行新未读：from=user，content=`smoke ping`
...
```bash
eduflow inbox manager       # 应有 4 条新未读，正文里包含原始 @
eduflow inbox worker_cc     # 应有 0 条新
eduflow inbox worker_codex  # 应有 0 条新
```
...
```bash
# 让 worker_cc 自己往群里发卡：
eduflow say worker_cc "auth 模块完成" --card
```
...
```bash
eduflow say manager "我同步一下进展" --card
```
...
1. **`state/router.log` 是新加的**（提交 `c0996a5`）。在它之前所有运行中失败都没有日志，要靠 `eduflow down && eduflow up` 重启清状态来恢复
