...
个 EduFlow"。`eduflow version` 在 stdout 打印一行 semver-ish
...
- EduFlow 通过 `pip install -e .` 安过（pyproject.toml `version`
...
- `eduflow version --help` → "usage: eduflow version" + exit 0

## Why this is here

跟 git rev-parse 性质类似 —— machine-readable 入口让 CI / smoke /
support tickets 不再用 `pip show eduflow | grep Version` 这种间
...
- **build hash / commit sha**：现在只读 pyproject.toml 的 version
  ...
  fallback 链。
- **`eduflow --version`**: 全局 `--version` flag 没实装；要走
  subcommand `eduflow version`。这是 sub-command 风格 CLI 的常见
  约定（kubectl / git 也一样）。
