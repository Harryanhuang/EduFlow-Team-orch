...
- 已通过 `eduflow status` / `eduflow log` 写过几条数据

## When

```bash
# 三个 agent 上报状态
eduflow status worker_a 进行中 "处理 X"
eduflow status worker_b 已完成 "Y 完成"
eduflow status worker_c 阻塞 "等审批" "需要老板批准 budget"

# worker_a 留下 5 条审计日志
for i in 1 2 3 4 5; do
  eduflow log worker_a info "step $i complete"
done

# 读
eduflow team
eduflow workspace worker_a
eduflow workspace worker_a --limit 2
```
