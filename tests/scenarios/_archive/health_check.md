# `eduflow health` — 部署状态一键快照

## 场景

`eduflow health` 给老板和救火的 oncall 一个清晰的"现在到底有没有跑起来"
...
eduflow up                  # 等价于 start + router + watchdog
eduflow health
```
...
2. **lark_profile 留空的 ⚠️**——只要 `EDUFLOW_LARK_SEND_AS=bot` 在
   shell env 里，发送链路其实工作正常；这条只是提醒以防有人没设环境变量

## 不在范围

- 看用量：`eduflow usage`，看 [usage_snapshot.md](usage_snapshot.md)
- 看团队成员状态：`eduflow team`，看 [team_overview_and_workspace.md](team_overview_and_workspace.md)
