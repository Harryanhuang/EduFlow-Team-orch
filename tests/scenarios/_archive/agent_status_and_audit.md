...
```bash
# status 三模式
eduflow status worker 进行中 "处理 X 任务"
eduflow status worker          # 读
eduflow status worker 阻塞 "等审批" "需要老板批准 budget"

# log 多条
eduflow log worker info "started X"
eduflow log worker task "完成 step 1" "TASK-7"
eduflow log worker error "step 2 失败"
```
