# Team teardown: `eduflow down` vs `eduflow reset`

## 场景

team 在跑，需要停。两条路：

- `eduflow down` — 优雅关停 daemons + tmux session，**保留 state**
  （inbox / status / cursor / identity 文件全留着）。重启 `eduflow up`
  能立刻接着干，inbox 历史和 router cursor 都还在。
- `eduflow reset` — 关停 + **wipe state**。inbox 清零、cursor 清零、
...
- `eduflow up` 已经把 team 拉起来过
- `$EDUFLOW_STATE_DIR` 下有 `facts/inbox.json`, `facts/status.json`,
...
🛑 tmux session EduFlow killed
✅ team down
```

之后 `ls $EDUFLOW_STATE_DIR/facts/` 仍然能看到 `inbox.json`，inbox
历史完整保留。`eduflow up` 重新启动后 router catchup 会从 cursor
继续读 Feishu，不丢消息。
...
之后 `ls $EDUFLOW_STATE_DIR/` → 目录不存在（被 rmtree 掉了）。
`team.json` 和 `runtime_config.json` 还在工作目录里。`eduflow up`
...
- `eduflow reset`（不带 `--yes` 且不在 TTY 里）→ exit 1, stderr `aborted`
- `eduflow down`（其实没什么在跑）→ exit 0，每行打印 `⏭ ...: no pid file`，
  最后 `✅ team down`
...
**state**，不是清 **config**。要重新换 team 用 `eduflow init --force`，
要切 team 用 `eduflow switch`。
...
- **保留 inbox 但清 cursor**：现在 reset 是全清。要细粒度的话手工
  `rm $EDUFLOW_STATE_DIR/router.cursor`，没 wrapper。
