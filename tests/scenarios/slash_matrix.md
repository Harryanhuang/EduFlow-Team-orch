...
对应的契约：人 → manager。manager 自己决定要不要再用 `eduflow send`
...
| # | 触发 | 期望 |
| --- | --- | --- |
| W1 | 在 worker_cc pane 里跑 `eduflow say worker_cc "auth 模块完成"` | 群里出现 worker_cc 的绿色卡；router 把这条卡路由到 manager 的收件箱（sender=worker_cc）；manager pane 处理这条新收件箱 |
| W2 | 同上但 sender 是 manager（`eduflow say manager "..."`） | 群里出现 manager 的卡；**没有任何 agent 收件箱**新增（router DROP `bot_self`） |

### Agent 之间互发（peer messaging）

下面这些跑在 agent pane 内部，验证 `eduflow send`：

| # | 起点 pane | 命令 | 期望 |
| --- | --- | --- | --- |
| P1 | manager | `eduflow send worker_cc manager "查 auth 模块"` | worker_cc 收件箱多一行（from=manager, to=worker_cc）；pane 收到正文。manager pane 不变 |
| P2 | worker_cc | `eduflow send manager worker_cc "auth 用 bcrypt"` | manager 收件箱多一行（from=worker_cc, to=manager）；manager pane 收到。**回归点**：from=worker_cc 不能被错当成跨团队或自言自语而 DROP |
| P3 | worker_cc | `eduflow send worker_codex worker_cc "看一下 token 过期处理"` | worker_codex 收件箱新增一行；pane 收到。peer-to-peer 不会镜像到 manager |
| P4 | manager | `eduflow say manager "进展同步" --card` | 群里出现 manager 蓝色卡头的 v2 markdown 卡；老板 + workers 在群 UI 里都能看到。**这条不写任何收件箱**——`say` 是「发到群里说一声」，不是「派单进收件箱」 |
...
| # | 准备 | 期望 |
| --- | --- | --- |
| V1 | worker_cc 跑 `eduflow say worker_cc "完成 auth 模块" --card` | 群里出现 worker_cc 卡（按 worker_* 配色绿色卡头）。**manager 收件箱也会拿到这条**（W1 反向路由分支）；manager 通过收件箱或 `eduflow peek worker_cc` 都能看 |
| V2 | manager 在自己 pane 里跑 `eduflow peek worker_cc 30` | 输出是 worker_cc pane 缓冲最近 30 行；和从群里发 `/tmux worker_cc 30` 等价但少一次往返 |
| V3 | 全员汇报：老板说 `@team 报告状态`；每个 worker 通过 `eduflow say <自己> "<状态>" --card`；manager 跑 `eduflow team` 看 | 每个 worker 都展示「进行中」或「已完成」；群里每个 worker 一张卡，老板能扫卡或 `/team` 看汇总 |

### 收件箱审计

| # | 步骤 | 期望 |
| --- | --- | --- |
| I1 | 老板说 `@worker_cc test`；R174 后这条到 manager 收件箱。再让 manager 通过 `eduflow send worker_cc manager test` 转给 worker_cc，然后在 worker_cc pane 里 `eduflow inbox worker_cc` | 看得到 from=manager 的一行 test |
| I2 | worker_cc 跑 `eduflow read <local_id>`，再 `eduflow inbox worker_cc --unread` | 那一行从未读列表消失 |
