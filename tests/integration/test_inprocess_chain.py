...
from eduflow.feishu import subscribe
...
def _fake_chat_send():
    ...
    from eduflow.feishu import chat as _chat_module
...
def test_human_message_lands_in_manager_inbox_and_pane():
    with _isolated(), _fake_inject() as inj:
        ...
        # agent posts replies via `eduflow say` instead of answering
        # in pane. The original message body still appears verbatim.
        assert "please help" in manager_inj[0]["text"]
        assert "eduflow say manager" in manager_inj[0]["text"]
...
def test_at_team_routes_to_manager_only_r174():
    ...
    intent and decides whether to fan-out via `eduflow send` per
...
def test_runtime_drill_rate_limit_switches_and_delivers():
    from helpers import attr_patch
    from eduflow.feishu import deliver as deliver_mod
    from eduflow.runtime import lifecycle as lifecycle_mod
    ...
    switched = []
    with _isolated(), _fake_inject() as inj, \
            tmux_patch(capture_pane=lambda t, lines=80: "Approaching usage limit\n"), \
    ...
    assert switched == [("worker_codex", "backup_codex", "deliver:rate_limit")]
    assert len(inj["calls"]) == 1


def test_runtime_drill_auth_failure_switches_and_delivers():
    from helpers import attr_patch
    from eduflow.feishu import deliver as deliver_mod
    from eduflow.runtime import lifecycle as lifecycle_mod
    ...
    switched = []
    with _isolated(), _fake_inject() as inj, \
            tmux_patch(capture_pane=lambda t, lines=80: "Invalid auth credentials\n"), \
...
def test_runtime_drill_provider_unavailable_switches_and_delivers():
    from helpers import attr_patch
    from eduflow.feishu import deliver as deliver_mod
    from eduflow.runtime import lifecycle as lifecycle_mod
    ...
    switched = []
    with _isolated(), _fake_inject() as inj, \
            tmux_patch(capture_pane=lambda t, lines=80: "503 service unavailable\n"), \
            attr_patch(deliver_mod.wake, is_rate_limited=lambda *a, **kw: False), \
            attr_patch(lifecycle_mod,
                       current_runtime_status=lambda agent: {"runtime": "primary"},
                       restart_with_runtime=lambda agent, target, runtime_name, reason="", prove_ready=False, **_kwargs: switched.append((agent, runtime_name, reason)) or "ready"), \
...
def test_lazy_pane_wake_fn_invoked_then_inject_proceeds():
    ...
    router only routes to manager from chat — but `eduflow send`
...
def test_rate_limited_pane_keeps_inbox_skips_inject():
    ...
    with _isolated(), _fake_inject() as inj, attr_patch(
        ...
        # (manager → worker via `eduflow send`) still constructs
