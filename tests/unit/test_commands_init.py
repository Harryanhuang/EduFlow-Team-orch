"""Tests for `eduflow init` — first-time bootstrap.

Post-config-unification: writes a single `eduflow.toml` instead of
...
def _tmp_env():
    """isolated_env points EDUFLOW_CONFIG_FILE at a fresh tempdir's
    eduflow.toml that doesn't exist yet (init writes it)."""
    return isolated_env()
...
def test_init_writes_toml_and_returns_zero():
    with _tmp_env() as tmp:
        ...
        assert rc == 0
        assert (tmp / "eduflow.toml").exists()
        assert "wrote" in out
        assert "Next:" in out
...
def test_init_default_team_has_three_agents():
    with _tmp_env() as tmp:
        with env_patch(EDUFLOW_CONFIG_FILE=str(tmp / "eduflow.toml")):
            run_cli(["init"])
        cfg = _read_toml(tmp / "eduflow.toml")
...
def test_init_refuses_to_overwrite_existing_toml():
    with _tmp_env() as tmp:
        toml_path = tmp / "eduflow.toml"
        toml_path.write_text('chat_id = "preserved"\n', encoding="utf-8")
        with env_patch(EDUFLOW_CONFIG_FILE=str(toml_path)):
...
def test_upgrade_merges_legacy_team_json():
    """Existing team.json + runtime_config.json → eduflow.toml with
...
def test_init_help_returns_zero():
    ...
    assert rc == 0
    assert "usage: eduflow init" in out
