"""Tests for src/eduflow/util.py — small shared helpers."""
...
def test_maybe_print_help_prints_and_returns_true_on_help_flag():
    ...
    with contextlib.redirect_stdout(out):
        assert maybe_print_help(["--help"], "usage: eduflow X") is True
    assert "usage: eduflow X" in out.getvalue()
...
def test_maybe_print_help_returns_false_without_help_flag():
    ...
    out = io.StringIO()
    with contextlib.redirect_stdout(out):
        assert maybe_print_help(["foo", "bar"], "usage: eduflow X") is False
    assert out.getvalue() == ""
