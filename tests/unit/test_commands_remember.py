"""Tests for `eduflow remember` — agent durable memory entry point."""
...
def test_remember_help_flag():
    ...
    assert rc == 0
    assert "usage: eduflow remember" in out
...
def test_remember_registered_in_cli():
    """Round-87: top-level `eduflow remember` must be in the COMMANDS
    registry; otherwise managers/workers calling from pane get
    `unknown command`."""
    from eduflow.cli import COMMANDS
