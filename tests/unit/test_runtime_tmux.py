...
from eduflow.runtime.tmux import (
    PaneInfo,
    Target,
    capture_pane,
    has_session,
...
def test_respawn_agent_replaces_pane_command():
    rec = _Recorder()
    assert respawn_agent(Target("S", "w"), "codex --model gpt-5.5", run=rec) is True
    assert rec.calls == [
        ["tmux", "respawn-pane", "-k", "-t", "S:w", "codex --model gpt-5.5"],
    ]
...
def test_list_panes_parses_tmux_output():
    rec = _Recorder([_FakeResult(
        returncode=0,
        stdout="0\t0\tzsh\t/bin/zsh\n1\t1\tnode\tOMX_SESSION_ID=1 node omx.js hud --watch\n",
    )])
    panes = list_panes(Target("S", "worker"), run=rec)
    assert panes == [
        PaneInfo(index="0", active=False, current_command="zsh", start_command="/bin/zsh"),
        PaneInfo(index="1", active=True, current_command="node",
                 start_command="OMX_SESSION_ID=1 node omx.js hud --watch"),
    ]


def test_preferred_pane_target_prefers_non_hud_pane():
    rec = _Recorder([_FakeResult(
        returncode=0,
        stdout="0\t0\tzsh\t/bin/zsh\n1\t1\tnode\tOMX_SESSION_ID=1 node omx.js hud --watch\n",
    )])
    target = preferred_pane_target(Target("S", "worker_qbank"), run=rec)
    assert str(target) == "S:worker_qbank.0"


def test_preferred_pane_target_keeps_single_pane_window():
    rec = _Recorder([_FakeResult(
        returncode=0,
        stdout="0\t1\tclaude\t/opt/homebrew/bin/claude\n",
    )])
    target = preferred_pane_target(Target("S", "manager"), run=rec)
    assert str(target) == "S:manager"
...
def test_default_run_returns_soft_failure_when_tmux_missing():
    ...
    every eduflow command that touches tmux. Now returns a soft
