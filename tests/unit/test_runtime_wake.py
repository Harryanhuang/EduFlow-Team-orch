...
def test_wake_respawns_stale_lazy_pane_before_polling():
    target = tmux.Target("S", "worker_builder")
    captures = [
    ...
    respawn_calls = []
    ok = wake.wake_if_dormant(
        target, _ClaudeFake(), spawn_cmd="claude --model sonnet",
        capture=capture,
        spawn=lambda t, c: spawn_calls.append(c) or True,
        respawn=lambda t, c: respawn_calls.append(c) or True,
        sleep=lambda s: None,
        timeout_s=5.0, poll_interval_s=0.1,
    )
    assert ok is True
    assert spawn_calls == []
    assert respawn_calls == ["claude --model sonnet"]
...
def test_wait_until_ready_accepts_bypass_permissions_yes_not_default_no():
    target = tmux.Target("S", "worker_builder")
    captures = [
        "❯ 1. No, exit\n  2. Yes, I accept\nEnter to confirm",
        "bypass permissions on\n>",
    ...
    sent = []
    with attr_patch(wake.tmux, send_keys=lambda target, *keys: sent.append(keys) or True):
        ok = wake.wait_until_ready(
            target, _ClaudeFake(),
            capture=_capturer(captures),
            sleep=lambda s: None,
...
def test_wait_until_ready_waits_for_full_bypass_options(monkeypatch):
    target = tmux.Target("S", "worker_builder")
    captures = [
    ...
    sent = []
    monkeypatch.setattr(wake.tmux, "send_keys", lambda target, *keys: sent.append(keys) or True)
    ok = wake.wait_until_ready(
        target, _ClaudeFake(),
        capture=_capturer(captures),
        sleep=lambda s: None,
...
def test_wait_until_ready_ignores_stale_dialog_above_shell_prompt():
    target = tmux.Target("S", "worker_builder")
    stale = (
        "❯ 1. No, exit\n  2. Yes, I accept\nEnter to confirm\n"
        "huanganan@host repo %\n"
    ...
    with attr_patch(wake.tmux, send_keys=lambda target, *keys: sent.append(keys) or True):
        ok = wake.wait_until_ready(
            target, _ClaudeFake(),
            capture=lambda target, lines=80: stale,
            sleep=lambda s: None,
            now=now,
