...
from eduflow.runtime.watchdog import (
    ProcessSpec,
    ProcessState,
    check_entrypoint_drift,
    classify_stability,
    default_specs,
    FLAP_THRESHOLD_PER_WINDOW,
    FLAP_WINDOW_SECS,
    is_alive,
    list_orphan_pids,
    record_restart,
...
def _spec(**overrides) -> ProcessSpec:
    base = dict(
        ...
        expected_cmdline="eduflow router",
        spawn_cmd=["eduflow", "router"],
        max_retries=3,
        cooldown_secs=600,
        orphan_markers=(),
    )
...
def test_is_alive_false_when_pid_dead():
    ...
    assert is_alive(
        ...
        read_cmdline=lambda pid: "eduflow router",
    ) is False
...
def test_is_alive_true_when_all_three_match():
    ...
    assert is_alive(
        ...
        read_pid=lambda p: 1234,
        pid_alive=lambda pid: True,
        read_cmdline=lambda pid: "python eduflow router --foo",
    ) is True


def test_is_alive_false_when_process_is_defunct():
    spec = _spec()
    assert is_alive(
        spec,
        read_pid=lambda p: 1234,
        pid_alive=lambda pid: True,
        read_cmdline=lambda pid: "<defunct>",
    ) is False


def test_default_specs_spawn_via_repo_python_entrypoint():
    specs = {spec.name: spec for spec in default_specs()}
    router = specs["router"]
    assert router.spawn_cmd[0] == sys.executable
    assert router.spawn_cmd[1:4] == ["-m", "eduflow.cli", "router"]
    assert router.expected_cmdline == "eduflow.cli"
...
def test_respawn_uses_devnull_when_log_file_unset():
    ...
    pythonpath_entries = captured["env"]["PYTHONPATH"].split(":")
    assert str(Path(__file__).resolve().parents[2] / "src") in pythonpath_entries
...
def test_default_specs_includes_router_pointing_at_state_dir():
    with isolated_env() as tmp:
        ...
        assert router.spawn_cmd == [sys.executable, "-m", "eduflow.cli", "router"]
...
def test_all_known_specs_router_has_orphan_markers_watchdog_does_not():
    ...
    from eduflow.runtime.watchdog import all_known_specs
    with isolated_env():
        specs = {s.name: s for s in all_known_specs()}
    assert specs["router"].orphan_markers
    assert specs["task-publish"].orphan_markers == ()
    assert specs["watchdog"].orphan_markers == ()


def test_default_specs_includes_router_and_task_publish_but_not_watchdog():
    ...
    knows about router + task-publish."""
    from eduflow.runtime.watchdog import default_specs as _default
    with isolated_env():
        names = [s.name for s in _default()]
    assert names == ["router", "task-publish"]


def test_all_known_specs_includes_router_task_publish_and_watchdog():
    """all_known_specs is the bigger list used by `eduflow health`
    (and `eduflow up` / `eduflow down`) to enumerate every
    daemon EduFlow ships. Includes the watchdog itself so health
    can verify its lock file matches a live process."""
    from eduflow.runtime.watchdog import all_known_specs
    with isolated_env() as tmp:
        specs = all_known_specs()
        names = sorted(s.name for s in specs)
        assert names == ["router", "task-publish", "watchdog"]
        # Both pid files live under the isolated state_dir
        for s in specs:
            assert str(s.pid_file).startswith(str(tmp))
        # spawn_cmd shape is [sys.executable, "-m", "eduflow.cli", <name>]
        for s in specs:
            assert s.spawn_cmd == [sys.executable, "-m", "eduflow.cli", s.name]
        # Both expect the module-entrypoint cmdline marker (defends against
        # PID reuse — see ProcessSpec.expected_cmdline)
        for s in specs:
            assert s.expected_cmdline == "eduflow.cli"


# ── stability classification ────────────────────────────────────────


def test_classify_stability_stable_by_default():
    """A fresh ProcessState with no restarts should be 'stable'."""
    st = ProcessState("test")
    assert classify_stability(st, now=1000.0) == "stable"
    assert st.flapping is False


def test_classify_stability_flapping_after_threshold_respawns():
    """When respawns exceed FLAP_THRESHOLD_PER_WINDOW within FLAP_WINDOW_SECS,
    classify_stability should return 'flapping'."""
    st = ProcessState("test")
    now = 1000.0
    # Add exactly FLAP_THRESHOLD_PER_WINDOW respawns within the window
    for i in range(FLAP_THRESHOLD_PER_WINDOW):
        st.restart_timestamps.append(now - 100 + i * 10)
    assert classify_stability(st, now=now) == "flapping"
    assert st.flapping is True


def test_classify_stability_stable_when_respawns_spread_out():
    """Respawns spread over >1 hour should remain 'stable'."""
    st = ProcessState("test")
    now = 10000.0
    # Add respawns spaced 2 hours apart (well beyond FLAP_WINDOW_SECS)
    for i in range(10):
        st.restart_timestamps.append(now - (i * 7200))
    assert classify_stability(st, now=now) == "stable"
    assert st.flapping is False


def test_classify_stability_drift_when_last_action_is_drift():
    """When st.last_action = 'drift', classify_stability returns 'drift'."""
    st = ProcessState("test")
    st.last_action = "drift"
    assert classify_stability(st, now=1000.0) == "drift"
    assert st.flapping is False


def test_classify_stability_inconsistent_when_last_action_is_inconsistent():
    """When st.last_action = 'inconsistent', classify_stability returns 'inconsistent'."""
    st = ProcessState("test")
    st.last_action = "inconsistent"
    assert classify_stability(st, now=1000.0) == "inconsistent"
    assert st.flapping is False


def test_record_restart_prunes_old_timestamps():
    """Old timestamps beyond FLAP_WINDOW_SECS should be pruned."""
    st = ProcessState("test")
    now = 10000.0
    # Add old timestamps outside the window
    st.restart_timestamps = [now - 5000, now - 4000, now - 3700]
    record_restart(st, now)
    # Only the new timestamp and any within the window should remain
    cutoff = now - FLAP_WINDOW_SECS
    assert all(t > cutoff for t in st.restart_timestamps)
    assert len(st.restart_timestamps) == 1  # just the one we added


def test_record_restart_keeps_timestamp_at_window_boundary():
    """The flapping window includes restarts exactly FLAP_WINDOW_SECS old."""
    st = ProcessState("test")
    now = 10000.0
    boundary = now - FLAP_WINDOW_SECS
    st.restart_timestamps = [boundary, boundary - 0.01]
    record_restart(st, now)
    assert st.restart_timestamps == [boundary, now]


def test_classify_stability_counts_timestamp_at_window_boundary():
    """Boundary timestamps should count toward the flapping threshold."""
    st = ProcessState("test")
    now = 10000.0
    st.restart_timestamps = [
        now - FLAP_WINDOW_SECS,
        now - 100,
        now - 90,
        now - 80,
        now - 70,
...
def test_check_entrypoint_drift_false_when_cmdline_matches():
    """PID alive, cmdline matches → no drift."""
    spec = _spec()
    assert check_entrypoint_drift(
        spec,
        read_pid=lambda p: 1234,
        pid_alive=lambda pid: True,
        read_cmdline=lambda pid: "eduflow router",
...
def test_check_entrypoint_drift_true_when_cmdline_mismatch():
    """PID alive, cmdline wrong → drift detected."""
    spec = _spec()
    assert check_entrypoint_drift(
        spec,
        read_pid=lambda p: 1234,
...
def test_check_entrypoint_drift_false_when_pid_dead():
    """PID dead → no drift (process gone, not drift)."""
    spec = _spec()
    assert check_entrypoint_drift(
        spec,
        read_pid=lambda p: 1234,
...
# ── supervise records restart timestamps ───────────────────────────


def test_supervise_records_restart_timestamps_on_respawn():
    """After supervise() triggers a respawn, st.restart_timestamps should have an entry."""
    spec = _spec()
    states: dict = {}
    respawn_times = []

    def fake_respawn(s):
        respawn_times.append(s.name)
        return True

    supervise([spec], states,
              alive_check=lambda s: False,
              respawn_fn=fake_respawn,
              now=lambda: 1000.0, log=lambda *_: None)

    assert len(respawn_times) == 1
    assert len(states["router"].restart_timestamps) == 1
    assert states["router"].restart_timestamps[0] == 1000.0


def test_supervise_does_not_record_restart_while_in_cooldown():
    """Cooldown sweeps should not inflate restart history."""
    spec = _spec()
    states = {
        "router": ProcessState(
            "router",
            cooldown_until=2000.0,
            restart_timestamps=[900.0],
        )
    }
    respawn_calls = []
    ...
    supervise([spec], states,
              alive_check=lambda s: False,
              respawn_fn=lambda s: respawn_calls.append(s.name) or True,
              now=lambda: 1000.0, log=lambda *_: None)

    assert respawn_calls == []
    assert states["router"].last_action == "cooldown"
    assert states["router"].restart_timestamps == [900.0]
