"""Tests for `eduflow switch` — multi-team env-export emitter."""
...
def test_switch_no_arg_prints_current_active():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with env_patch(EDUFLOW_STATE_DIR=str(sd),
                       EDUFLOW_TEAM_FILE=str(tf),
                       EDUFLOW_RUNTIME_CONFIG=str(rt)):
...
def test_switch_no_arg_prints_defaults_when_env_unset():
    ...
    with env_patch(EDUFLOW_STATE_DIR=None,
                   EDUFLOW_TEAM_FILE=None,
                   EDUFLOW_RUNTIME_CONFIG=None):
        rc, out, _ = run_cli(["switch"])
    assert rc == 0
...
def test_switch_help_returns_zero():
    ...
    assert rc == 0
    assert "usage: eduflow switch" in out
...
def test_switch_expands_tilde():
    """`eduflow switch ~/teams/x` should expand the tilde before
...
def test_switch_rejects_dir_without_team_json():
    ...
    assert "eduflow init" in err  # hint to next step
