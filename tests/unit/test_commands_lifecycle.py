"""Tests for `eduflow start / hire / fire` — team lifecycle commands.
...
from eduflow.commands import start as start_cmd
...
def test_start_prints_runtime_name_when_agent_uses_runtime_registry():
    with _isolated_team({"session": "T", "agents": {"w": {"runtime": "primary"}}}) as tmp:
...
""", encoding="utf-8")
        ...
        with _fake_tmux():
            rc, out, _ = run_cli(["start"])
        assert rc == 0
        assert "runtime=primary" in out


def test_start_propagates_state_dir_into_pane_env():
    """REGRESSION: round 4 smoke caught that worker_cc's \`eduflow say\`
    wrote to ~/.eduflow/facts/logs.jsonl instead of the project state
    dir, because tmux send-keys spawned the CLI in a fresh shell that
    didn't inherit EDUFLOW_STATE_DIR. Spawn line must prepend it."""
...
def test_start_adds_settle_sleep_after_claude_agents():
    team = {
        "session": "T",
        "agents": {
    ...
    with _isolated_team(team), _fake_tmux(), \
        ...
        stack.enter_context(contextlib.redirect_stdout(__import__("io").StringIO()))
        stack.enter_context(contextlib.redirect_stderr(__import__("io").StringIO()))
        real_sleep = start_cmd.time.sleep
        start_cmd.time.sleep = lambda s: sleeps.append(s)
        try:
            rc = start_cmd.main([])
        finally:
            start_cmd.time.sleep = real_sleep
    assert rc == 0
...
""", encoding="utf-8")
        from eduflow.runtime import tunables
        tunables.reset_cache()
        with _fake_tmux() as fake:
            fake["session_exists"].add("S")
            rc, out, _ = run_cli(["hire", "new"])
    assert rc == 0
    assert "hired: new (codex-cli)" in out


def test_hire_ready_timeout_returns_one_instead_of_fake_success():
    team = {"session": "S", "agents": {"manager": {}, "new": {"cli": "claude-code"}}}
    with _isolated_team(team), _fake_tmux() as fake, tmux_patch(
            capture_pane=lambda target, lines=80: "$ "):
        fake["session_exists"].add("S")
        rc, _, err = run_cli(["hire", "new"])
        assert rc == 1
        assert "didn't show ready marker" in err
        assert "hired: new" not in err
