...
def test_human_at_mention_still_routes_only_to_manager():
    ...
    Manager decides whether to dispatch via `eduflow send`."""
...
def test_agent_prefix_with_no_target_drops():
    ...
    largely dead since `eduflow say` posts cards now). The card
...
def test_slash_strips_sender_prefix_before_detection():
    """REGRESSION (round A2 B1): \`eduflow say boss "/team"\` produces
...
def test_worker_card_say_routes_to_manager_inbox():
    """Worker `eduflow say` posts an interactive card with title
