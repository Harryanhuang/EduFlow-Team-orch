...
def test_render_manager_uses_manager_template():
    ...
    # 老板 → manager → eduflow send → workers. The identity now
    # spells out the dispatch flow + visibility into worker say replies.
    assert "唯一接口" in text
    assert "eduflow send" in text
    # Argument-order contract carried over from rebuild's earlier version
    assert "eduflow send <recipient> <sender>" in text


def test_render_manager_includes_eduflow_team_dispatch_entry():
    text = identity.render("manager", role="团队主管",
                           cli="claude-code", model="opus")
    assert "EduFlow Team 固定入口" in text
    assert "message-intent-gate" in text
    assert "manager-dispatch-orchestrator" in text
    assert "manager-task-triage" in text
    assert "workflow-route-jump" in text
    assert "role-direct-dispatch" in text
...
def test_render_worker_uses_worker_template():
    ...
    assert "--ack accepted_task" in text
    assert "--ack started_task" in text
    assert "在岗外显协议" in text
    assert "阶段进度：<已做/正在做/下一步>" in text
    assert "已交接：<产物/对象>" in text
    assert "worker_qbank" in text
    assert "qbank 校验已开始" in text
...
def test_render_argument_order_contract_present_in_manager():
    ...
    assert "eduflow send <recipient> <sender>" in text
    assert "eduflow say <agent>" in text
...
def test_manager_identity_dispatch_step_uses_to_user():
    ...
    assert 'eduflow say manager "<已派给 N 位...>" --to user' in text
...
def test_init_prompt_omits_memory_section_when_empty():
    ...
    with isolated_env():
        ...
        assert "eduflow inbox manager" in prompt
...
def test_init_prompt_teaches_inbox_processing_after_R168():
    ...
    with isolated_env():
        ...
        # Tells agent to use eduflow say for status reports
        assert "eduflow say worker_cc" in prompt
        # Tells agent to leave a real ACK, not just plain read
        assert "eduflow read" in prompt
        assert "--ack accepted_task" in prompt
        assert "--ack accepted_revision" in prompt
        assert "--ack started_task" in prompt
        assert "qbank 当前卡在" in prompt


def test_init_prompt_does_not_force_ready_status_before_inbox_processing():
    with isolated_env():
        prompt = identity.init_prompt("worker_cc")
    assert 'eduflow status worker_cc 进行中 "ready"' not in prompt
    assert 'eduflow status worker_cc 进行中 "<current task>"' in prompt
