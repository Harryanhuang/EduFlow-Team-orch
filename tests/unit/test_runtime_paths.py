...
def _state_env(value):
    ...
    return env_patch(EDUFLOW_STATE_DIR=value)


def test_state_dir_falls_back_to_home_when_env_unset():
    with _state_env(None):
        assert paths.state_dir() == Path.home() / ".eduflow"
...
def test_named_pid_files_land_in_state_dir():
    with tempfile.TemporaryDirectory() as tmp:
        with _state_env(tmp):
            ...
            assert paths.task_publish_cursor_file() == Path(tmp) / "task-publish.cursor"
            assert paths.task_publish_pid_file() == Path(tmp) / "task-publish.pid"
            assert paths.task_publish_log_file() == Path(tmp) / "task-publish.log"
