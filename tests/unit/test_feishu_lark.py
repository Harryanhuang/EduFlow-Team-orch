...
def test_run_builds_lark_cli_argv_with_profile():
    ...
    with env_patch(EDUFLOW_LARK_CLI_BIN="/usr/local/bin/lark-cli"):
...
def test_resolve_cli_prefix_uses_explicit_env_override():
    """Round-86: EDUFLOW_LARK_CLI_BIN takes priority over auto-discovery."""
    import tempfile
    import os as _os
    with tempfile.TemporaryDirectory() as td:
        ...
        with env_patch(EDUFLOW_LARK_CLI_BIN=fake_bin):
...
def test_resolve_cli_prefix_ignores_nonexistent_override():
    ...
    with env_patch(EDUFLOW_LARK_CLI_BIN="/does/not/exist/lark-cli"):
        prefix = lark.resolve_cli_prefix()
...
def test_resolve_cli_prefix_falls_back_to_npx_when_nothing_else():
    ...
    try:
        ...
        with env_patch(EDUFLOW_LARK_CLI_BIN=""):
            prefix = lark.resolve_cli_prefix()
    finally:
...
def test_nonzero_exit_with_json_blob_extracts_real_message():
    """Smoke v3 found: when eduflow say targets an unjoinable chat,
...
def test_run_keeps_proxy_env_when_no_proxy_unset():
    ...
    HTTPS_PROXY must be preserved. Pin EDUFLOW_CONFIG_FILE to a
    non-existent path so the project root's eduflow.toml (which
    ...
    with env_patch(HTTPS_PROXY="http://x", LARK_CLI_NO_PROXY=None,
                   EDUFLOW_CONFIG_FILE="/nonexistent/eduflow.toml"):
        lark.call(["x"], run=rec)
...
def test_subprocess_env_pins_home_to_pw_dir():
    ...
    `eduflow say`, the lark-cli subprocess inherits that per-agent
...
def test_timeout_default_is_90s_when_unset():
    """No explicit timeout, no EDUFLOW_LARK_TIMEOUT → 90 (matches the
    docstring; lark-cli routinely takes ~73s on host networks per
    project_lark_cli_slow.md memory)."""
    rec = _Recorder(FakeProc(stdout="{}"))
    with env_patch(EDUFLOW_LARK_TIMEOUT=None):
        lark.call(["x"], run=rec)
    assert rec.calls[0]["kwargs"]["timeout"] == 90
...
def test_timeout_clamps_zero_or_negative_to_one():
    ...
    rec = _Recorder(FakeProc(stdout="{}"))
    with env_patch(EDUFLOW_LARK_TIMEOUT="0"):
        lark.call(["x"], run=rec)
    assert rec.calls[0]["kwargs"]["timeout"] == 1
    rec2 = _Recorder(FakeProc(stdout="{}"))
    with env_patch(EDUFLOW_LARK_TIMEOUT="-5"):
...
def test_run_returns_none_when_npx_not_on_path():
    ...
    installed) used to propagate as a top-level traceback through
    every eduflow say / chat invocation. Now caught with a
    one-liner pointing at Node.js install."""
    import io
    import contextlib
