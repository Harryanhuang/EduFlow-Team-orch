"""Tests for the top-level eduflow CLI dispatcher."""
...
def test_handler_unhandled_exception_prints_friendly_error():
    ...
    Python traceback. Set EDUFLOW_DEBUG=1 to see the trace."""
    def handler(argv):
    ...
    assert "EDUFLOW_DEBUG=1" in err
    # No traceback by default
    assert "Traceback" not in err


def test_handler_unhandled_exception_with_debug_env_reraises():
    """When EDUFLOW_DEBUG=1 is set, the original exception propagates
    ...
    try:
        with env_patch(EDUFLOW_DEBUG="1"):
            try:
                run_cli(["debug-boom"])
