...
def test_docker_stats_filters_to_claudeteam_prefix():
    ...
    are present (e.g. `eduflow-foo-team-1` → `foo`); the simpler
    case `eduflow-team-1` keeps the bare `-team-1` since the
    suffix isn't preceded by a `-`."""
    stats_out = (
        '{"Name":"eduflow-foo-team-1","CPUPerc":"5.2%","MemPerc":"35.1%",'
        '"MemUsage":"500MiB / 2GiB"}\n'
        '{"Name":"unrelated-service-1","CPUPerc":"99.9%","MemPerc":"99.9%",'
        '"MemUsage":"7GiB / 8GiB"}\n'
        '{"Name":"eduflow-other-1","CPUPerc":"1.0%","MemPerc":"5.0%",'
        '"MemUsage":"100MiB / 2GiB"}\n'
    )
    ps_out = (
        "eduflow-foo-team-1\tUp 5 hours\n"
        "unrelated-service-1\tUp 1 day\n"
        "eduflow-other-1\tUp 2 hours\n"
    )
    ...
    assert names == ["eduflow-foo-team-1", "eduflow-other-1"]
    team = next(c for c in out if c["name"] == "eduflow-foo-team-1")
    assert team["short"] == "foo"  # eduflow-foo-team-1 → foo
...
def test_alarms_fire_above_thresholds():
    ...
    full_disk = {"mount": "/", "pct": 92, "used": 460 * 1024**3,
                 "total": 500 * 1024**3}
    pressure_container = {"name": "eduflow-x", "short": "x",
...
def test_alarms_silent_when_under_thresholds():
    ...
    healthy_disk = {"mount": "/", "pct": 40, "used": 200 * 1024**3,
                    "total": 500 * 1024**3}
    quiet_container = {"name": "eduflow-x", "short": "x",
