"""Tests for `eduflow say` — Feishu chat send + local mirror."""
...
def _fake_send():
    ...
    R169: `eduflow say` is card-only; the old text path is dead.
...
def test_say_sends_to_chat_and_logs_locally():
    ...
    with _isolated(), _fake_send() as send:
        ...
        assert rc == 0
        assert "manager → main chat (message_id=om_fake)" in out
        assert send["calls"]
        call = send["calls"][0]
        ...
        assert any(log["type"] == "say" and log["content"] == "hello world" for log in logs)
...
def test_say_env_var_picks_user_when_no_flag():
    with _isolated(), _fake_send() as send, \
            env_patch(EDUFLOW_LARK_SEND_AS="user"):
        run_cli(["say", "manager", "hi", "--no-card"])
        assert send["calls"][0]["as_user"] is True
...
def test_say_supervisor_channel_uses_dedicated_chat_and_profile():
...
lark_profile = "supervisor-profile"
sender_name = "Hermes Supervisor"
sender_role = "外部监督通道"
sender_emoji = "🛡️"
sender_color = "red"
""".strip() + "\n",
        ...
        rc, out, _ = run_cli([
            "say", "manager", "supervision ping",
            "--channel", "supervisor",
        ])
    assert rc == 0
    assert "→ supervisor chat" in out
    assert send["calls"][0]["chat_id"] == "oc_supervisor"
    assert send["calls"][0]["profile"] == "supervisor-profile"
    card = send["calls"][0]["card"]
    assert card["header"]["title"]["content"] == "🛡️ Hermes Supervisor · 外部监督通道"
    assert card["header"]["template"] == "red"


def test_say_supervisor_channel_falls_back_to_main_when_blank():
    with _isolated(chat_id="oc_main", profile="main-profile"), _fake_send() as send:
        rc, out, _ = run_cli([
            "say", "manager", "supervision ping",
            "--channel", "supervisor",
        ])
    assert rc == 0
    assert "→ supervisor chat" in out
    assert send["calls"][0]["chat_id"] == "oc_main"
    assert send["calls"][0]["profile"] == "main-profile"
    card = send["calls"][0]["card"]
    assert card["header"]["title"]["content"] == "🎯 manager · 团队主管"


def test_say_unknown_channel_returns_one():
...
def test_say_card_color_reflects_live_toml_edit():
    """REGRESSION: editing a worker's `card_color` in eduflow.toml
    ...
    with _isolated() as tmp, _fake_send_card() as st:
        ...
        # Operator edits eduflow.toml — flip worker_cc to red
...
def test_say_default_now_sends_card_after_R168():
    """R168: default flipped — every `eduflow say` now sends a v2
...
def test_say_body_flag_keeps_agent_as_first_positional_arg():
    """Agents sometimes call `say worker --body ...`; the body flag must not
    be parsed as the agent, or local visibility lands under `--body`.
    """
    with _isolated(), _fake_send() as send:
        rc, _, _ = run_cli([
            "say",
            "worker_cc",
            "--body",
            "[worker_cc] Batch revision complete. Ready for review.",
            "--to",
            "manager",
        ])
        assert rc == 0
        ...
        assert "Batch revision complete" in logs[0]["content"]
        assert local_facts.list_logs("--body") == []
...
def _toml_with_publish(tmp_path, **kv):
    """Drop a eduflow.toml with [chat.publish] = kv into tmp_path."""
    from eduflow.runtime import tunables
    lines = ["[chat.publish]"]
    for k, v in kv.items():
        if v == "always":
    ...
    (tmp_path / "eduflow.toml").write_text("\n".join(lines) + "\n", encoding="utf-8")
    tunables.reset_cache()
...
def test_say_publish_live_edit_takes_effect_without_restart():
    """REGRESSION: editing eduflow.toml [chat.publish] should
...
def test_say_allows_qbank_process_visibility_when_worker_to_user_silenced():
    with _isolated() as tmp, _fake_send() as send:
        _toml_with_publish(tmp, worker_to_user=False)
        rc, out, _ = run_cli([
            "say",
            "worker_cc",
            "题库验证全量扫描已开始：正在生成统一 manifest 和验证报告。",
            "--to",
            "user",
    ])
    assert rc == 0
...
def test_say_to_arg_value_required():
    ...
    assert rc == 1
    assert "usage: eduflow say" in err
...
def test_say_hermes_worker_like_visibility_can_override_global_silence():
    with _isolated_with_overrides("Hermes", {"worker_to_user": True}) as tmp, \
            _fake_send() as send:
        _toml_with_publish(tmp, worker_to_user=False)
        rc, out, _ = run_cli([
            "say",
            "Hermes",
            "知识库任务已接单：整理 Memory candidate backlog",
            "--to",
            "user",
        ])
        assert rc == 0
...
# ── Package 8: agent card color mapping ────────────────────────────
# _color_for must return a stable colour for every known agent name
# regardless of team-config order. Unknown agents fall back to blue.


def test_agent_card_color_manager_is_blue():
    from eduflow.commands.say import _color_for
    assert _color_for("manager") == "blue"


def test_agent_card_color_worker_course_is_purple():
    from eduflow.commands.say import _color_for
    assert _color_for("worker_course") == "purple"


def test_agent_card_color_review_course_is_green():
    from eduflow.commands.say import _color_for
    assert _color_for("review_course") == "green"


def test_agent_card_color_auto_ops_is_red():
    from eduflow.commands.say import _color_for
    assert _color_for("auto_ops") == "red"


def test_agent_card_color_worker_builder_is_orange():
    from eduflow.commands.say import _color_for
    assert _color_for("worker_builder") == "orange"


def test_agent_card_color_Anna_is_yellow():
    from eduflow.commands.say import _color_for
    assert _color_for("Anna") == "yellow"


def test_agent_card_color_worker_qbank_is_turquoise():
    from eduflow.commands.say import _color_for
    assert _color_for("worker_qbank") == "turquoise"


def test_agent_card_color_unknown_agent_falls_back_to_blue():
    from eduflow.commands.say import _color_for
    assert _color_for("nonexistent_bot_xyz") == "blue"
    assert _color_for("") == "blue"
    assert _color_for("random_user") == "blue"
