"""Tests for `eduflow install-hooks` — Claude Code slash-command markdowns."""
...
def test_install_hooks_peek_md_documents_5min_cadence():
    """Round-104: /peek hook teaches `eduflow peek <agent> [N]` as
...
def test_install_hooks_remember_md_documents_kind_vocabulary():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for kind in ("task_assigned", "task_completed", "learning",
            ...
            assert kind in body
        assert "eduflow remember" in body
...
def test_install_hooks_say_md_documents_card_only_after_R169():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "eduflow say <your-name>" in body
...
def test_install_hooks_help():
    ...
    assert rc == 0
    assert "usage: eduflow install-hooks" in out
...
def test_install_hooks_warns_when_session_already_running():
    ...
    \`eduflow up\` is the wrong order; existing claude-code panes
    have already cached their slash commands and won't pick up the
    new files. install-hooks should warn loudly."""
    team = {"session": "EduFlow", "agents": {"manager": {}}}
    with isolated_env(team=team) as tmp, \
            tmux_patch(has_session=lambda s: s == "EduFlow"):
        rc, _, err = run_cli(["install-hooks", str(tmp)])
        assert rc == 0
        # warning lands on stderr (via util.warn)
        assert "tmux session 'EduFlow' is already running" in err
        assert "eduflow down && eduflow up" in err
