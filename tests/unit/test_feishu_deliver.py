...
from eduflow.feishu import deliver as deliver_mod
from eduflow.feishu.deliver import (
...
def test_rate_limited_pane_switches_runtime_when_fallback_exists():
    decision = Decision(action=Action.ROUTE, targets=["worker_a"], text="x", msg_id="om")
    inject_calls = []
    send_key_calls = []
    respawn_calls = []
    team = {"agents": {"worker_a": {"runtime": "primary"}}}
...
""", encoding="utf-8")
        ...
        class ReadyAdapter(RateLimitedAdapter):
            def rate_limit_markers(self):
                return []

        adapters = iter([RateLimitedAdapter(), ReadyAdapter()])

        def factory(_agent):
            return next(adapters)

        with tmux_patch(
                capture_pane=lambda t, lines=80: "fake-ready\nApproaching usage limit\n",
                ...
                attr_patch(deliver_mod.wake, wait_until_ready=lambda *a, **kw: True):
            report = apply(
                decision,
    ...
    assert "eduflow inbox worker_a" in inject_calls[0][1]
    assert "恢复后先处理最新 inbox" in inject_calls[1][1]
    assert inject_calls[2][1].endswith("\n\nx")


def test_auth_failure_switches_runtime_before_inject():
    decision = Decision(action=Action.ROUTE, targets=["worker_a"], text="x", msg_id="om")
    inject_calls = []
    respawn_calls = []
    team = {"agents": {"worker_a": {"runtime": "primary"}}}
...
""", encoding="utf-8")
        ...
        with tmux_patch(
                capture_pane=lambda t, lines=80: "Invalid auth credentials\n",
...
def test_route_prefers_non_hud_pane_when_window_has_multiple_panes():
    ...
    with tmux_patch(
            list_panes=lambda target: [
                ...
                type("Pane", (), {
                    ...
                    "start_command": "OMX_SESSION_ID=1 node omx.js hud --watch",
                })(),
            ],
            inject=lambda target, text, submit_keys=None: inject_calls.append(str(target)) or True):
        with isolated_env(team=_WAKE_TEAM):
            report = apply(
                decision,
                adapter_for_agent=_adapter_factory,
    ...
    assert report.injected == ["worker_a"]
    assert inject_calls == ["S:worker_a.0"]
...
def test_compose_inject_text_includes_local_id_for_mark_read():
    ...
    `eduflow read <id>` so the agent clears its inbox after replying."""
    out = _compose_inject_text(
        "worker_cc", _decision("ack me"), local_id="msg_42")
    assert "eduflow read msg_42" in out
...
def test_compose_inject_text_summary_cue_skipped_for_manager_self():
    ...
    # The base "eduflow say manager" hint stays
    assert "eduflow say manager" in out
    # But the extra "send manager" line is suppressed for manager itself
    assert "eduflow send manager manager" not in out
...
def test_compose_inject_text_summary_cue_skipped_without_keyword():
    ...
    assert "eduflow say worker_cc" in out
    assert "eduflow send manager" not in out
