"""Tests for `eduflow router` daemon entry.
...
def test_build_cmd_does_NOT_use_force_anymore():
    ...
    is fcntl-advisory and auto-releases on process exit; eduflow's
...
def test_main_help_returns_zero():
    ...
    assert rc == 0
    assert "usage: eduflow router" in out
...
def test_watch_subscribe_health_self_terminates_on_stale_events():
    ...
    try:
        ...
        # eduflow.toml.
        with env_patch(EDUFLOW_ROUTER_STALE_S="0.1",
                       EDUFLOW_ROUTER_SUBSCRIBE_WATCHDOG_PERIOD_S="0.05"):
...
def test_watch_subscribe_health_self_terminates_on_child_exit():
    ...
    try:
        ...
        with env_patch(EDUFLOW_ROUTER_STALE_S="3600",
                       EDUFLOW_ROUTER_SUBSCRIBE_WATCHDOG_PERIOD_S="0.05"):
            stop_event = threading.Event()
...
def test_load_seen_truncates_huge_file_to_recent_window():
    ...
    from eduflow.runtime import paths
    cap = 50
    with isolated_env(), env_patch(EDUFLOW_ROUTER_SEEN_MAX_LINES=str(cap)):
        paths.ensure_state_dir()
