...
async function detectConfiguredCdpEndpoint(rawEndpoint) {
  const endpoint = normalizeCdpEndpoint(rawEndpoint);
  if (!endpoint) return { ok: false, endpoint: rawEndpoint || null, reason: 'invalid_endpoint' };
  try {
    const res = await fetch(`${endpoint}/json/version`, { signal: AbortSignal.timeout(3000) });
    if (!res.ok) return { ok: false, endpoint, reason: `http_${res.status}` };
    const info = await res.json();
    if (!info.webSocketDebuggerUrl) return { ok: false, endpoint, reason: 'missing_websocket_url' };
    ...
    return {
      ...
      browser: {
        id: 'remote-cdp',
        label: `Remote CDP (${endpointUrl.host})`,
        port,
        wsPath: null,
    ...
    return { ok: false, endpoint, reason: e.message || 'unreachable' };
  }
}
...
export async function selectBrowser(override = null) {
  ...
  const configuredEndpoint = cfg.WEB_ACCESS_CDP_ENDPOINT || null;
  if (configuredEndpoint) {
    const remote = await detectConfiguredCdpEndpoint(configuredEndpoint);
    if (remote.ok) {
      return {
        kind: 'ok',
        ...
        source: 'cdp_endpoint',
        detected: [],
        configured: cfg.WEB_ACCESS_BROWSER || null,
        cdpEndpoint: remote.browser.cdpEndpoint,
      };
    }
    return {
      kind: 'cdp_mismatch',
  ...
  const configured = cfg.WEB_ACCESS_BROWSER || null;
