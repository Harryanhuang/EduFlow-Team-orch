...
const DOWNLOAD_PATH = process.env.CDP_DOWNLOAD_PATH || '/downloads';
...
async function discoverChromePort() {
  ...
  if (result.kind === 'ok') {
    ...
    const tag = result.source === 'override'
      ? '[--browser 指定]'
      : result.source === 'cdp_endpoint'
        ? '[WEB_ACCESS_CDP_ENDPOINT]'
        : '[config.env 偏好]';
    console.log(`[CDP Proxy] 选用 ${result.browser.label} (端口 ${result.browser.port}${result.browser.wsPath ? '，带 wsPath' : ''}) ${tag}`);
    return { port: result.browser.port, wsPath: result.browser.wsPath, wsUrl: result.browser.wsUrl || null };
  }
  if (result.kind === 'cdp_mismatch') {
    throw new Error(
      `WEB_ACCESS_CDP_ENDPOINT 未连上: ${result.cdpEndpoint || '(empty)'} (${result.reason || 'unknown'}). ` +
      `Agent 处理顺序：(1) docker compose up -d agent-chrome；` +
      `(2) curl -s http://127.0.0.1:9223/json/version；` +
      `(3) 若端口不同，同步修改 config.env 的 WEB_ACCESS_CDP_ENDPOINT。`
    );
  }
...
function getWebSocketUrl(port, wsPath, wsUrl = null) {
...
let downloadBehaviorReady = false;

async function ensureDownloadBehavior() {
  if (downloadBehaviorReady) return;
  if (connectedBrowser?.id !== 'remote-cdp' && !process.env.CDP_DOWNLOAD_PATH) return;
  try {
    await sendCDP('Browser.setDownloadBehavior', {
      behavior: 'allow',
      downloadPath: DOWNLOAD_PATH,
    });
    downloadBehaviorReady = true;
  } catch (e) {
    console.error('[CDP Proxy] 下载目录设置失败:', e.message);
  }
}
