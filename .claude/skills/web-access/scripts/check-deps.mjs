...
async function resolveAndReport(override) {
  ...
  switch (result.kind) {
    ...
    case 'cdp_mismatch': {
      console.log(`browser: error — WEB_ACCESS_CDP_ENDPOINT 未连上: ${result.cdpEndpoint || '(empty)'}`);
      console.log(`  reason: ${result.reason || 'unknown'}`);
      console.log('  Agent 处理顺序：');
      console.log('    1. 先启动专用浏览器容器：docker compose up -d agent-chrome');
      console.log('    2. 再检查 CDP：curl -s http://127.0.0.1:9233/json/version');
      console.log('    3. 若端口被占用，修改 docker-compose.yml 的宿主端口和 config.env 的 WEB_ACCESS_CDP_ENDPOINT 保持一致');
