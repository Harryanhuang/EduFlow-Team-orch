...
## Local Routing On This Machine

On this machine, use the same daemon endpoint `http://127.0.0.1:10086`. It is intentionally connected to the Docker Chrome service in the EduFlow project, not to a normal desktop Chrome window:

- Docker service: `agent-chrome` in `/Volumes/Halobster/Obsidian Edu/留学公司知识库/11-Eduflow Team 多智能体项目/EduFlow-Team-orch/docker-compose.yml`
- Docker DevTools endpoint: `http://127.0.0.1:9233`
- Browser extension ID: `fldmhceldgbpfpkbgopacenieobmligc`
- Container bridge: Docker Chrome exposes `127.0.0.1:10086` inside the container through a TCP proxy to `host.docker.internal:10086`, so the extension auto-connects to the host daemon.

Before browser work, check `~/.kimi-webbridge/bin/kimi-webbridge status`. A healthy Docker Chrome route shows `extension_connected:true` and `extension_id:"fldmhceldgbpfpkbgopacenieobmligc"`. If the daemon is down, start it with `~/.kimi-webbridge/bin/kimi-webbridge start`; if the extension is disconnected, run `docker compose up -d agent-chrome` from the EduFlow project root.
