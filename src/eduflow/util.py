...
def atomic_write_text(path: Path, content: str, *, encoding: str = "utf-8") -> None:
    ...
    Uses a PID-suffixed temp file (`<path>.<pid>.tmp`) so concurrent
    writers (e.g. two router instances racing on cursor updates) each
    get their own temp file instead of clobbering each other's and
    failing the rename with ENOENT.

    Creates parent directories if missing. Stale temp files from
    crashed processes are cleaned up on a best-effort basis.
    """
    import os
...
def env_str(name: str) -> str:
    ...
    if val:
        return val
    if name.startswith("EDUFLOW_"):
        return os.environ.get("EDUFLOW_" + name[len("EDUFLOW_"):], "").strip()
    return ""
