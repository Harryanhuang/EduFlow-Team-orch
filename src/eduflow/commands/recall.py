"""`eduflow recall <agent> [--limit N] [--kind K] [--json]`

Print an agent's durable memory entries. Symmetric to `eduflow remember`.

Use cases:
  - operator audit: `eduflow recall manager` to see what manager has
    been remembering across /clear cycles.
  - manager 巡视 a worker: `eduflow recall worker_cc` from manager's
  ...
  - kind filter: `eduflow recall manager --kind decision` to scan
...
USAGE = (
    "usage: eduflow recall <agent> [--limit N] [--kind K] [--json]\n"
