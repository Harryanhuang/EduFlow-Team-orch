"""`eduflow init [--session NAME] [--force] [--upgrade]`

First-time bootstrap: writes `eduflow.toml` (the unified config file
...
in cwd, merges them into a `eduflow.toml`, leaves the originals as
backup. Lets existing deployments migrate without losing their team
config.

Refuses to overwrite an existing `eduflow.toml` unless --force.
"""
...
USAGE = "usage: eduflow init [--session NAME] [--force] [--upgrade]"
...
# EduFlow 配置（单文件替代 team.json + runtime_config.json）
# 每个字段都可被同名 env var 覆盖：
#   EDUFLOW_<PATH>_<KEY>  例 router.stale_event_threshold_s
...
[feishu.supervisor]
chat_id          = ""  # 可选：Hermes / supervisor 专用问题反馈群
lark_profile     = ""  # 可选：单独 supervisor bot/profile；空则复用主 profile
...
def _upgrade_from_legacy(session: str) -> str:
    ...
    into a single eduflow.toml string. Caller is responsible for
    writing it.
...
def main(argv: list[str]) -> int:
    ...
    session = pop_flag(rest, "--session") or "EduFlow"
    ...
    if upgrade:
        ...
        print("  - eduflow install-hooks   # write .claude/commands/*.md")
        print(f"  - eduflow up              # tmux session '{session}' + router + watchdog")
        print("  - eduflow health          # verify green")
    return 0
