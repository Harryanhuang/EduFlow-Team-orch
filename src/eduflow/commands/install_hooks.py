"""`eduflow install-hooks` — drop slash-command markdowns for Claude Code agents.
...
command wired to the corresponding `eduflow` subcommand. Live set:
...
_HEAD = """\
You are a EduFlow agent. Read $EDUFLOW_STATE_DIR/agents/<your-name>/identity.md
...
as `worker_kimi` in round 7 smoke and call `eduflow say worker_kimi`.)
...
_COMMANDS: dict[str, str] = {
    "inbox": (
        "Run `eduflow inbox <your-name>` to list unread messages. "
        "Acknowledge each with `eduflow read <local_id>` once you start work on it.\n"
    ),
    "team": (
        "Run `eduflow team` to see every agent's status + heartbeat. "
        "Use this before delegating to confirm targets are alive.\n"
    ),
    "status": (
        "Run `eduflow status <your-name> <state> <task>` where state is one of "
    ...
    "say": (
        ...
        "\n"
        "Every `eduflow say` posts a v2 card with a color-coded header\n"
        ...
        "\n"
        "    eduflow say <your-name> \"【报道】当前状态：在线 ✅，正在做 X\"\n"
        "    eduflow say <your-name> \"收到\"\n"
        "    eduflow say <your-name> \"完工：登录页 /app/login.html 已交付\"\n"
        "\n"
        "Cards don't thread (`--reply <id>` is silently ignored).\n"
    ),
    "task": (
        "Manage the task tracker:\n"
        "- `eduflow task list` to see open work\n"
        "- `eduflow task create <assignee> <title>` to add\n"
        "- `eduflow task done <T-id>` when finished\n"
    ),
    "health": (
        "Run `eduflow health` and summarize: any red checks? any agent with "
    ...
    "remember": (
        ...
        "Run `eduflow remember <your-name> <kind> \"<content>\" [--ref <ref>]` "
    ...
    "peek": (
        "Run `eduflow peek <agent> [N]` to see another agent's last N pane "
        ...
        "Output is plain text; pipe to grep / less / `eduflow remember "
        "<your-name> note \"$(eduflow peek <agent> 5)\"` to record what "
        "you saw.\n"
    ),
    "recall": (
