"""`eduflow forget <agent> [--kind K] [--yes]` — wipe agent memory.

Symmetric to `eduflow recall` + `remember`. Used when an agent's
...
accumulated context. The reset command (`eduflow reset`) already
...
USAGE = (
    "usage: eduflow forget <agent> [--kind K] [--yes]\n"
...
def main(argv: list[str]) -> int:
    ...
    if not yes:
        ...
        recall_hint = (f"eduflow recall {agent} --kind {kind}" if kind
                       else f"eduflow recall {agent}")
        return error_exit(
