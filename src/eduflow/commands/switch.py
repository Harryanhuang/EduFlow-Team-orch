"""`eduflow switch <team-dir>` — print shell exports for a team directory.

Multi-team isolation today is env-var-based: a deployment is whichever
`team.json` + `runtime_config.json` + `EDUFLOW_STATE_DIR` the current
...
The directory layout this assumes (created either by `eduflow init`
in that dir or by hand) is:

    <team-dir>/
        team.json
        runtime_config.json
        state/                # auto-created when eduflow writes anything
...
from eduflow.util import env_str, error_exit, maybe_print_help


USAGE = (
    "usage: eduflow switch [<team-dir>]\n"
...
def _show_current() -> int:
    ...
    state = env_str("EDUFLOW_STATE_DIR") or f"(default) {paths.state_dir()}"
    team = env_str("EDUFLOW_TEAM_FILE") or f"(default) {config.team_file()}"
    rt = env_str("EDUFLOW_RUNTIME_CONFIG") or f"(default) {config.runtime_config_file()}"
...
def _emit_exports(team_dir: Path) -> int:
    ...
    if not team_json.exists():
        return error_exit(
            ...
            f"\n   (run `eduflow init` inside that directory first)")
    state_dir = team_dir / "state"
    rt_json = team_dir / "runtime_config.json"
    print(f"export EDUFLOW_STATE_DIR={shlex.quote(str(state_dir))}")
    print(f"export EDUFLOW_TEAM_FILE={shlex.quote(str(team_json))}")
    print(f"export EDUFLOW_RUNTIME_CONFIG={shlex.quote(str(rt_json))}")
    print(f"# Active team: {team_dir}")
    print(f"# Apply with: eval \"$(eduflow switch {team_dir})\"")
    return 0
