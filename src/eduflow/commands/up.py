"""`eduflow up` — bring the whole team alive in one shot.
...
`eduflow <name>` directly to see the actual error message.
"""
from __future__ import annotations

import time

from eduflow.commands import start as _start
from eduflow.runtime import config, tmux, watchdog
from eduflow.util import error_exit, maybe_print_help
...
def _ensure_daemon(spec: watchdog.ProcessSpec) -> int:
    ...
    # masking the boot failure from `eduflow up`'s exit code.
    for _ in range(30):
    ...
    return error_exit(
        ...
        f"likely fast-failed at startup; check `eduflow {spec.name}` "
        f"directly to see the error")


def main(argv: list[str]) -> int:
    if maybe_print_help(argv, "usage: eduflow up"):
        return 0
    ...
    if rc == 0:
        print("✅ team up — run `eduflow health` to verify")
    else:
        print("⚠️  team up with errors — see above; "
              "`eduflow health` will list which daemons died")
    return rc
