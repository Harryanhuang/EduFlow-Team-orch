"""`eduflow reset` — tear down the team and wipe its runtime state.

Deletes everything under `$EDUFLOW_STATE_DIR`: facts/inbox, status,
...
from eduflow.commands import down as _down
from eduflow.runtime import paths
from eduflow.util import error_exit, maybe_print_help, pop_bool_flag, reject_extra_args


USAGE = "usage: eduflow reset [--yes]"
...
def _confirm() -> bool:
    if not sys.stdin.isatty():
        return False
    try:
        ans = input("⚠️  delete all runtime state under $EDUFLOW_STATE_DIR? [y/N] ")
