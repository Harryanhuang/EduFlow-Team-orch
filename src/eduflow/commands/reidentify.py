"""`eduflow reidentify <agent>` — single agent
`eduflow reidentify --all`   — every agent in team.json with a live pane
...
USAGE = "usage: eduflow reidentify <agent>  |  eduflow reidentify --all"
...
def _reidentify_one(agent: str, session: str, *,
    ...
    # OLD specialty / role / notes. Edits to eduflow.toml only
...
def main(argv: list[str]) -> int:
    ...
    # picks up new eduflow.toml fields, not the snapshot at spawn.
    try:
        identity.write(agent)
    except Exception as e:
