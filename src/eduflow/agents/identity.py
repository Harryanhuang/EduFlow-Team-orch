...
- **context 强制恢复协议（必须执行）**：
  ...
  - 强制命令模板（按 affected agent 替换 `<agent>`；该命令会向目标 pane 注入 literal `/compact`，不是普通文字消息）：
    ```bash
    eduflow compact <agent>
    ```
  - 如果你在飞书 slash 通道，也可以用 `/compact <agent>`；**禁止**用 `eduflow send <agent> manager "请压缩 context..."` 这类文字提醒替代 `/compact`。
  - 发出后用 `eduflow status manager 进行中 "等待 <agent> context 恢复确认"` 记录，并在下一次 5 分钟 cadence 检查 `<agent>` 是否已回报；未回报则重试 `eduflow compact <agent>`、`eduflow reidentify <agent>` 或重启/重派小批次。
