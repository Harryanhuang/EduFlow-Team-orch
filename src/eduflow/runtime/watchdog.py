...
class ProcessState:
    ...
    # Rolling window of last N respawn epoch-times. Used by
    # `classify_stability()` to detect flapping (rapid restart cycles).
    # Pruned to entries within `FLAP_WINDOW_SECS` on every update.
    restart_timestamps: list[float] = field(default_factory=list)
    # True when restart frequency exceeds `FLAP_THRESHOLD_PER_WINDOW`
    # within `FLAP_WINDOW_SECS`. Set by `classify_stability()`.
    flapping: bool = False


# ── stability classification ────────────────────────────────────────

# How far back to look when computing restart frequency.
FLAP_WINDOW_SECS: float = 3600.0  # 1 hour
# Respawns within FLAP_WINDOW_SECS at or above this count mark flapping.
FLAP_THRESHOLD_PER_WINDOW: int = 5


def record_restart(st: ProcessState, now: float) -> None:
    """Append a restart timestamp and prune stale entries.

    Called by `supervise()` on every successful respawn. Keeps
    `restart_timestamps` bounded to `FLAP_WINDOW_SECS` so memory
    doesn't grow unbounded across long-running watchdog processes.
    """
    st.restart_timestamps.append(now)
    cutoff = now - FLAP_WINDOW_SECS
    st.restart_timestamps = [t for t in st.restart_timestamps if t >= cutoff]


def classify_stability(st: ProcessState, now: float) -> str:
    """Return one of: 'stable', 'flapping', 'drift', 'inconsistent'.

    - 'stable': no recent restarts, or restarts within threshold.
    - 'flapping': restart count within FLAP_WINDOW_SECS reaches threshold.
    - 'drift': process alive but cmdline doesn't match expected entrypoint
      (caller must detect this separately via `check_entrypoint_drift()`;
      this function only returns 'drift' if st.last_action == 'drift').
    - 'inconsistent': pid file present, process alive, but health probe
      disagrees (caller sets st.last_action = 'inconsistent' externally).

    Mutates `st.flapping` as a side effect for callers that want the
    bool without re-computing.
    """
    if st.last_action == "drift":
        st.flapping = False
        return "drift"
    if st.last_action == "inconsistent":
        st.flapping = False
        return "inconsistent"
    cutoff = now - FLAP_WINDOW_SECS
    recent = [t for t in st.restart_timestamps if t >= cutoff]
    st.flapping = len(recent) >= FLAP_THRESHOLD_PER_WINDOW
    return "flapping" if st.flapping else "stable"


# ── liveness check ────────────────────────────────────────────────


def _resolve_ps_binary() -> str:
    """Find the `ps` binary even when PATH is incomplete.

    Some shell configs leave $PATH unexpanded in env, so subprocess
    can't find `ps` by name. Try shutil.which first, then common
    absolute paths on macOS and Linux.
    """
    import shutil
...
def _read_cmdline(pid: int) -> str:
    try:
        ...
        # macOS fallback — resolve ps binary to survive broken PATH.
        # -ww prevents truncation of long Python paths (e.g.
        # /opt/homebrew/opt/python@3.14/.../python3.14 /opt/homebrew/bin/eduflow router)
        # which can exceed 80 chars and get clipped, cutting off "eduflow"
        # from the output and causing false-negative liveness checks.
        r = subprocess.run([_resolve_ps_binary(), "-ww", "-p", str(pid), "-o", "command="],
...
def is_alive(spec: ProcessSpec, *,
    ...
    if not cmdline or "<defunct>" in cmdline:
        return False
    return spec.expected_cmdline in cmdline


def check_entrypoint_drift(spec: ProcessSpec, *,
                           read_pid: Callable = pidlock.read_pid,
                           pid_alive: Callable = pidlock.pid_alive,
                           read_cmdline: Callable = _read_cmdline) -> bool:
    """Return True when the pid file points to a live process whose
    cmdline does NOT contain `spec.expected_cmdline`.

    This is a PID-reuse / drift condition: something else has taken
    over the pid, or the daemon was replaced by a different binary.
    Health should report `router_entrypoint_drift` instead of `alive`.

    Returns False when: pid file missing, process dead, or cmdline
    matches (i.e., no drift).
    """
    pid = read_pid(spec.pid_file)
    if pid is None or not pid_alive(pid):
        return False
    ...
    if not cmdline or "<defunct>" in cmdline:
        return False
    return spec.expected_cmdline not in cmdline
...
def list_orphan_pids(markers: tuple[str, ...], *,
    ...
    if not markers:
        return []
    try:
        r = run(["ps", "-ww", "-eo", "pid,ppid,command"],
...
def respawn(spec: ProcessSpec, *,
    ...
    too — `eduflow up`'s test rig relies on that.
    """
    reap(spec)
...
# ── default specs for EduFlow ──────────────────────────────────


def _eduflow_spec(name: str, pid_file: Path, *,
                  orphan_markers: tuple[str, ...] = (),
                  log_file: Path | None = None) -> ProcessSpec:
    """Build a ProcessSpec for a repo-local `python -m eduflow.cli <name>` daemon.

    We intentionally avoid depending on a globally installed `eduflow`
    executable being present on PATH. `eduflow up` / watchdog respawn should
    execute the current repo's code under the current interpreter.
    """
    return ProcessSpec(
        name=name,
        pid_file=pid_file,
        expected_cmdline="eduflow.cli",
        spawn_cmd=[
            sys.executable,
            "-m",
            "eduflow.cli",
...
def default_specs() -> list[ProcessSpec]:
    ...
    return [
        _eduflow_spec("router", paths.router_pid_file(),
                      orphan_markers=_ROUTER_SUBSCRIBE_MARKERS,
                      log_file=paths.router_log_file()),
        _eduflow_spec("task-publish", paths.task_publish_pid_file(),
                      log_file=paths.task_publish_log_file()),
    ]


def all_known_specs() -> list[ProcessSpec]:
    """Every daemon EduFlow ships, for `health` and similar audits.
    ...
    return [
        ...
        _eduflow_spec("task-publish", paths.task_publish_pid_file(),
                      log_file=paths.task_publish_log_file()),
        _eduflow_spec("watchdog", paths.watchdog_pid_file(),
