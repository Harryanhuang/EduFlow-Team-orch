...
def _default_run(args, **kwargs):
    ...
    Catches:
      - FileNotFoundError: tmux binary not on PATH (fresh container,
        derived Docker image without tmux). eduflow health flags
...
def list_panes(target: Target, *, run: Callable = _default_run) -> list[PaneInfo]:
    r = run([
        "tmux", "list-panes", "-t", str(target),
        "-F", "#{pane_index}\t#{pane_active}\t#{pane_current_command}\t#{pane_start_command}",
    ])
    if r.returncode != 0:
        return []
    panes: list[PaneInfo] = []
    for line in (r.stdout or "").splitlines():
        ...
        if len(parts) != 4:
            continue
        idx, active, current_cmd, start_cmd = parts
        panes.append(PaneInfo(
            index=idx,
            active=(active == "1"),
            current_command=current_cmd.strip(),
            start_command=start_cmd.strip(),
        ))
    return panes
...
def preferred_pane_target(target: Target, *, run: Callable = _default_run) -> Target:
    ...
    OMX may add an active HUD pane to a window. When we inject into the window
    target blindly, tmux sends to the active pane, which can be the HUD rather
    than the agent CLI/shell. Prefer a non-HUD pane when multiple panes exist.
    """
    try:
        panes = list_panes(target, run=run)
    except TypeError:
        # Older tests / monkeypatches may replace list_panes with a simpler
        # lambda(target) shape. Fall back so preferred-pane routing stays
        # compatible without forcing every test double to accept `run=`.
        panes = list_panes(target)
    if len(panes) <= 1:
        return target

    def _is_hud(p: PaneInfo) -> bool:
        start = p.start_command.lower()
        return "omx.js' hud --watch" in start or " omx.js hud --watch" in start

    non_hud = [p for p in panes if not _is_hud(p) and p.current_command != "node"]
    chosen = None
    if non_hud:
        chosen = next((p for p in non_hud if p.active), non_hud[0])
    else:
        chosen = next((p for p in panes if not _is_hud(p)), panes[0])
    return Target(target.session, f"{target.window}.{chosen.index}")
...
def respawn_agent(target: Target, spawn_cmd: str, *,
                  run: Callable = _default_run) -> bool:
    """Replace the pane process with a fresh shell command.

    Runtime fallback means the current CLI is degraded, so typing a new
    command into that UI can leave the pane half-recovered. `respawn-pane -k`
    kills the old pane command first and starts the replacement directly.
    """
    return _ok(["tmux", "respawn-pane", "-k", "-t", str(target), spawn_cmd], run)
