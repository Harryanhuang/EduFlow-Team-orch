"""Read tunable parameters from eduflow.toml with env-var override.

Cascade priority for every field:

  1. environment variable (e.g. `EDUFLOW_ROUTER_STALE_EVENT_THRESHOLD_S`)
  2. corresponding entry in `eduflow.toml`  (resolved via `paths.config_file()`)
...
Why a new module instead of folding into `runtime/config.py`:
  - config.py is the team.json / runtime_config.json reader (legacy json).
    tunables.py is the new parameterization layer that reads eduflow.toml.
...
# → env "EDUFLOW_ROUTER_STALE_EVENT_THRESHOLD_S"
def _env_var_for(dotted_path: str) -> str:
    return "EDUFLOW_" + dotted_path.upper().replace(".", "_")
...
def _load_toml() -> dict:
    """Read `eduflow.toml` from `paths.config_file()`. Returns `{}` if
...
def _coerce(value: str, target_type: type) -> Any:
    ...
    (e.g. EDUFLOW_ROUTER_STALE_EVENT_THRESHOLD_S=potato but field is
...
def tunable(dotted_path: str, default: Any) -> Any:
    ...
    Priority: env var > eduflow.toml > default.
...
def load() -> dict:
    """Public accessor for the parsed eduflow.toml. Returns `{}` if
