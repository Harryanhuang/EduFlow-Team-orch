...
plenty of margin; bump via `EDUFLOW_LARK_TIMEOUT` only if your
...
def subprocess_env() -> dict[str, str]:
    ...
    isolation; without this pin, `eduflow say` from inside an agent
...
def resolve_cli_prefix() -> list[str]:
    ...
    Resolution order (first hit wins):
      1. `EDUFLOW_LARK_CLI_BIN` env — operator explicit override.
    ...
    override = env_str("EDUFLOW_LARK_CLI_BIN")
    if not override:
        from eduflow.runtime import tunables
...
def _resolve_timeout(explicit: int | None) -> int:
    ...
    routes through tunables (env > eduflow.toml > default 90).

    Round-64: clamp the final value to >=1 — a garbage env like
    EDUFLOW_LARK_TIMEOUT=0 used to make subprocess.run
    insta-TimeoutExpired on every call. Legacy `EDUFLOW_LARK_TIMEOUT`
    env var still honored as backwards-compat alias.
    """
    if explicit is not None:
        return max(1, int(explicit))
    legacy = env_str("EDUFLOW_LARK_TIMEOUT").strip()
    if legacy:
        try:
