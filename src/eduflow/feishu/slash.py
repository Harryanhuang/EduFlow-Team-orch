...
Supported commands (matches main's 9-command surface):
    ...
    /home                              boss home card (team/action/risk)
    /sophon                            Sophon ops-watch card
    ...
    /dispatch <agent> <stage> <title>  create + assign a formal flow-task
...
class SlashContext:
    ...
    # eduflow.toml edits take effect without restarting the
    ...
    run: Callable = subprocess.run         # for shell-out (`eduflow <cmd>`)
...
def _live_agents(ctx: SlashContext | None = None) -> tuple[list[str], frozenset[str], frozenset[str], dict]:
    ...
    ctx.agent_set / ctx.lazy_agents so editing eduflow.toml takes
    ...
    - If live config looks like an evolution of the startup ctx set
      (one set is a subset of the other), trust live config.
    - If live config is a completely different team shape (common in
      unit tests that don't isolate cwd config), fall back to ctx.
    """
    from eduflow.runtime import config as _config
    agents_dict = _config.load_team().get("agents", {})
    names = list(agents_dict.keys())
    if ctx is not None and ctx.team_agents:
        ctx_names = list(ctx.team_agents)
        live_set = frozenset(names)
        ctx_set = frozenset(ctx_names)
        if names and not (ctx_set.issubset(live_set) or live_set.issubset(ctx_set)):
            return (
                ctx_names,
                ctx_set,
                frozenset(ctx.lazy_agents),
                {name: {} for name in ctx_names},
            )
...
/home                    → 老板首页：团队/任务/风险/下一动作
/sophon                  → Sophon 运营值守快照
...
def _handle_help(args: str, ctx: SlashContext) -> dict:
    ...
    return simple_card("🆘 EduFlow 自定义斜杠命令", _HELP_TEXT)
...
def _handle_team(args: str, ctx: SlashContext) -> dict:
    ...
    """
    # _live_agents reads config every call so eduflow.toml edits
    ...
    for agent in team_agents:
        # Fired agents (status="已停止" via `eduflow fire`) have no pane;
...
def _handle_usage(args: str, ctx: SlashContext) -> dict:
    ...
    elements.append({"tag": "markdown",
                      "content": (f"<font color='grey'>采集 "
                                   ...
                                   f"data source `eduflow usage --json`"
                                   f"</font>")})
...
def _handle_tmux(args: str, ctx: SlashContext) -> str | dict:
    ...
    Empty pane gets a placeholder, mirroring the CLI `eduflow
...
def _handle_dispatch(args: str, ctx: SlashContext) -> str:
    if not args.strip():
        return ("用法: /dispatch <agent> <stage> <title>\n"
                "例: /dispatch worker_cc curriculum Draft Unit 1")
    parts = args.split(None, 2)
    if len(parts) < 3:
        return ("用法: /dispatch <agent> <stage> <title>\n"
                "例: /dispatch worker_cc curriculum Draft Unit 1")
    agent, stage, title = parts[0].strip(), parts[1].strip(), parts[2].strip()
    if (warn := _bad_agent(agent, ctx)):
        return warn
    if not title:
        return "⚠️ dispatch 缺少任务标题"
    try:
...
def _handle_submit(args: str, ctx: SlashContext) -> str:
    if not args.strip():
        return ("用法: /submit <task_id> <agent>\n"
                "例: /submit T-1 worker_cc")
    parts = args.split(None, 1)
    if len(parts) < 2:
        return ("用法: /submit <task_id> <agent>\n"
                "例: /submit T-1 worker_cc")
    task_id, agent = parts[0].strip(), parts[1].strip()
    if (warn := _bad_agent(agent, ctx)):
        return warn
    try:
        ok = tasks.submit_for_review(task_id, actor=agent)
    except ValueError as e:
        return f"⚠️ submit 失败: {e}"
    if not ok:
        return f"⚠️ submit 失败: no such task: {task_id}"
    row = tasks.get(task_id) or {}
    return (
        f"✅ /submit 已提交审核 {task_id} → {agent} "
        f"[{row.get('stage', '-')}] {row.get('title', '')}"
    ).rstrip()


def _handle_assign_reviewer(args: str, ctx: SlashContext) -> str:
    if not args.strip():
        return ("用法: /assign-reviewer <task_id> <reviewer>\n"
                "例: /assign-reviewer T-1 reviewer_amy")
    parts = args.split(None, 1)
    if len(parts) < 2:
        return ("用法: /assign-reviewer <task_id> <reviewer>\n"
                "例: /assign-reviewer T-1 reviewer_amy")
    task_id, reviewer = parts[0].strip(), parts[1].strip()
    if (warn := _bad_agent(reviewer, ctx)):
        return warn
    try:
        ok = tasks.assign_reviewer(task_id, reviewer=reviewer, actor="manager")
    except ValueError as e:
        return f"⚠️ assign-reviewer 失败: {e}"
    if not ok:
        return f"⚠️ assign-reviewer 失败: no such task: {task_id}"
    return f"✅ /assign-reviewer 已指派审核人 {reviewer} → {task_id}"


def _handle_review_queue(args: str, ctx: SlashContext) -> dict:
    reviewer = args.strip() or None
    rows = tasks.list_review_queue(reviewer=reviewer)
    title = f"📥 待审核队列 [{ctx.session}] {beijing_stamp(ctx.now)}"
    if not rows:
        body = "**待审核队列**\n当前没有待审核任务。"
        if reviewer:
            body = f"**待审核队列（{reviewer}）**\n当前没有待审核任务。"
        return simple_card(title, body, color="green")

    lines = ["**待审核队列**"]
    if reviewer:
        lines = [f"**待审核队列（{reviewer}）**"]
    for row in rows[:10]:
        lines.append(
    ...
    if len(rows) > 10:
        lines.append(f"<font color='grey'>… 另有 {len(rows) - 10} 项未显示</font>")
    return simple_card(title, "\n".join(lines), color="yellow")


def _handle_manager_overview(args: str, ctx: SlashContext) -> dict:
    overview = tasks.manager_overview()
    title = f"📊 经理总览 [{ctx.session}] {beijing_stamp(ctx.now)}"
    labels = {
        "in_progress": "进行中",
        "awaiting_review": "待审核",
        "blocked": "阻塞中",
        "manager_action": "需经理处理",
    }
    lines = ["**经理总览**"]
    for key in ("in_progress", "awaiting_review", "blocked", "manager_action"):
        rows = overview[key]
        lines.append(f"**{labels[key]} {len(rows)}**")
        for row in rows[:3]:
            lines.append(
                f"- `{row['id']}` [{row.get('stage') or '-'}] {row['title']} "
...
def _task_window_line(row: dict) -> str:
    line = f"　`{row['id']}` {row.get('title') or '(无标题)'}"
    bits = []
    for key in ("stage", "owner", "assignee", "reviewer"):
        value = row.get(key)
        if value:
            bits.append(f"{key}={value}")
    if row.get("verdict") and row.get("verdict") != "pending":
        bits.append(f"verdict={row['verdict']}")
    if bits:
        line += " · " + " · ".join(bits[:4])
    summary = tasks.flow_live_summary(row) if row.get("schema_version") == 2 else ""
    if summary:
        line += f"\n　　{summary[:80]}"
    return line
...
def _handle_task(args: str, ctx: SlashContext) -> dict:
    """/task [all] -> read-only task window; complex decisions stay in manager-panel."""
    expand = args.strip().lower() in ("all", "全部")
    rows = tasks.list_tasks()
    buckets: dict[str, list[dict]] = {status: [] for status, _, _ in _TASK_COLUMNS}
    for row in rows:
        buckets.setdefault(str(row.get("status") or ""), []).append(row)

    terminal = set(tasks.FLOW_TERMINAL_STATUSES) | set(tasks.TERMINAL_STATUSES)
    folded = False
    body_lines: list[str] = []
    for status, glyph, label in _TASK_COLUMNS:
        group = buckets.get(status, [])
        if not group:
            continue
        fold = not expand and status in terminal
        body_lines.append(f"{glyph} **{label}**（{len(group)}）" + ("▸ 已折叠" if fold else ""))
        if fold:
            folded = True
            continue
        body_lines.extend(_task_window_line(row) for row in group[:8])
        if len(group) > 8:
            body_lines.append(f"　… 另有 {len(group) - 8} 项未显示")
    ...
    attention = any(buckets.get(status) for status in (
        "submitted_for_review", "blocked", "failed",
    )) or any(row.get("verdict") == "manager_action" for row in rows)
    return simple_card(
        f"🗂️ /task — 任务窗口 [{ctx.session}] {beijing_stamp(ctx.now)}",
        "\n".join(body_lines),
        color="yellow" if attention else "green",
    )
...
def _handle_home(args: str, ctx: SlashContext) -> dict:
    """/home → boss home card backed by the ops dashboard."""
    dashboard = _load_ops_dashboard(ctx)
    return team_snapshot_card(
        dashboard,
        title_suffix=f"home [{ctx.session}] {beijing_stamp(ctx.now)}",
    )


def _handle_sophon(args: str, ctx: SlashContext) -> dict:
    """/sophon → Sophon ops-watch card backed by the ops dashboard."""
    dashboard = _load_ops_dashboard(ctx)
    return team_snapshot_card(
        dashboard,
        title_suffix=f"Sophon [{ctx.session}] {beijing_stamp(ctx.now)}",
    )
...
_HANDLERS: dict[str, Callable[[str, SlashContext], str]] = {
    ...
    # Back-compat aliases; help points users to /home and /sophon.
    "/ops": _handle_sophon,
    "/ops-dashboard": _handle_sophon,
    ...
    "/assign-reviewer": _handle_assign_reviewer,
    "/review-queue": _handle_review_queue,
    "/manager-overview": _handle_manager_overview,
    "/task": _handle_task,
    "/compact": _handle_compact,
    "/stop": _handle_stop,
    "/clear": _handle_clear,
}
# 19 chat slash commands: /help /team /home /sophon /employees /employee
# /ops /ops-dashboard /health /usage /tmux /send
